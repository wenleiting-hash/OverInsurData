...
let testResults = {
  ...
  failed: 0,
  skipped: 0,
  tests: [] as Array<{ name: string; status: 'PASS' | 'FAIL' | 'SKIP'; message: string }>
};
...
// 工具函数：添加测试结果
function addTest(name: string, status: 'PASS' | 'FAIL' | 'SKIP', message: string) {
...
const hasRTLSupport = configContent.includes("'ar-SA'") || configContent.includes("'he-IL'");

addTest(
  'config.ts includes RTL languages',
  hasRTLSupport ? 'PASS' : 'FAIL',
  hasRTLSupport 
    ? '✅ Arabic and Hebrew language codes detected in configuration' 
    : '❌ RTL languages NOT configured - need to update i18n config'
);
...
function extractKeys(obj: any, prefix = ''): string[] {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (typeof value === 'object' && !Array.isArray(value)) {
      keys = keys.concat(extractKeys(value as any, fullKey));
    } else {
      keys.push(fullKey);
    }
  }
...
function compareKeys(enKeys: string[], zhKeys: string[]): { missingInZH: string[]; missingInEN: string[]; match: string[] } {
