...
export default function ComplianceRulesView({ navigateTo }: Props) {
  ...
  const categoryLabels: Record<RuleCategory, string> = {
    appointment: t('rules.categories.appointment'),
    license: t('rules.categories.license'),
    ofac: t('rules.categories.ofac'),
    channel: t('rules.categories.channel'),
    product: t('rules.categories.product'),
  };

  const actionLabels: Record<string, { bg: string; text: string; label: string }> = {
    block: { bg: 'bg-red-100', text: 'text-red-700', label: t('rules.actions.block') },
    warn: { bg: 'bg-orange-100', text: 'text-orange-700', label: t('rules.actions.warn') },
    'require-review': { bg: 'bg-yellow-100', text: 'text-yellow-700', label: t('rules.actions.requireReview') },
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="glass p-6 rounded-xl">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-600">{t('rules.stats.total')}</span>
          </div>
        ...
        <div className="glass p-6 rounded-xl">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-600">{t('rules.stats.active')}</span>
          </div>
      ...
      <div className="mb-6 glass px-4 py-2 rounded-t-lg inline-flex gap-1 flex-wrap">
        {[
          { key: 'all', label: t('rules.tabs.all') },
          { key: 'appointment', label: t('rules.categories.appointment') },
          { key: 'license', label: t('rules.categories.license') },
          { key: 'ofac', label: t('rules.categories.ofac') },
          { key: 'channel', label: t('rules.categories.channel') },
          { key: 'product', label: t('rules.categories.product') },
        ].map(tab => (
          <button
            key={tab.key}
            ...
            <div className="relative bg-white rounded-lg p-6 max-w-2xl w-full shadow-xl">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="appointment">{t('rules.categories.appointment')}</option>
                    <option value="license">{t('rules.categories.license')}</option>
                    <option value="ofac">{t('rules.categories.ofac')}</option>
                    <option value="channel">{t('rules.categories.channel')}</option>
                    <option value="product">{t('rules.categories.product')}</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="block">{t('rules.form.actionBlock')}</option>
                    <option value="warn">{t('rules.form.actionWarn')}</option>
                    <option value="require-review">{t('rules.form.actionRequireReview')}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="mb-6">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-sm font-semibold text-gray-700">{t('rules.form.enabled')}</span>
                </label>
              </div>
