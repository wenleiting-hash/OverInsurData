...
const STEPS = [
  { labelKey: 'new.steps.channel.label', subKey: 'new.steps.channel.sub',   icon: ShieldCheck },
  { labelKey: 'new.steps.insurer.label', subKey: 'new.steps.insurer.sub',   icon: Building2 },
  { labelKey: 'new.steps.details.label', subKey: 'new.steps.details.sub',   icon: MapPin },
  { labelKey: 'new.steps.confirm.label', subKey: 'new.steps.confirm.sub',   icon: FileText },
]
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          >
            <ArrowLeft size={14} /> {t('new.backToCompliance')}
          </button>
        ...
        <div className="card" style={{ padding: '20px 16px', position: 'sticky', top: 0 }}>
              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: '#A0A5B4', marginTop: 1 }}>{t(s.subKey)}</div>
                  </div>
                </div>
              )
            })}
          ...
          {(selectedChannel || selectedInsurer) && (
            <div style={{ marginTop: 18, paddingTop: 14, borderTop: '0.5px solid rgba(193,198,215,0.4)' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#A0A5B4', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>{t('new.selectedInfo')}</div>
              {selectedChannel && (
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: '#A0A5B4', marginBottom: 2 }}>{t('new.labels.channel')}</div>
              ...
              {selectedInsurer && (
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: '#A0A5B4', marginBottom: 2 }}>{t('new.labels.insurer')}</div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#181C23' }}>{selectedInsurer.name}</div>
                </div>
              )}
              {form.state && (
                <div>
                  <div style={{ fontSize: 11, color: '#A0A5B4', marginBottom: 2 }}>{t('new.labels.stateLine')}</div>
        ...
        <div>
          <div className="card" style={{ padding: '28px 32px' }}>
            ...
            {step === 0 && (
              <div>
                <div style={{ marginBottom: 18 }}>
                  <h2 style={{ fontSize: 16, fontWeight: 800, color: '#181C23', margin: '0 0 6px' }}>{t('new.step0.title')}</h2>
                  <p style={{ fontSize: 13, color: '#717786' }}>{t('new.step0.subtitle')}</p>
                </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 12, color: '#717786', marginTop: 3, fontFamily: "'JetBrains Mono', monospace" }}>
                              NPN: {ch.npn} · {ch.state} · {t('new.agentsCount', { n: ch.agents })}
                            </div>
                          </div>
                          {active && (
            ...
            {step === 1 && (
              <div>
                <div style={{ marginBottom: 18 }}>
                  <h2 style={{ fontSize: 16, fontWeight: 800, color: '#181C23', margin: '0 0 6px' }}>{t('new.step1.title')}</h2>
                  <p style={{ fontSize: 13, color: '#717786' }}>{t('new.step1.subtitle')}</p>
                </div>
            ...
            {step === 2 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
                <div style={{ marginBottom: 2 }}>
                  <h2 style={{ fontSize: 16, fontWeight: 800, color: '#181C23', margin: '0 0 6px' }}>{t('new.step2.title')}</h2>
                  <p style={{ fontSize: 13, color: '#717786' }}>{t('new.step2.subtitle')}</p>
                </div>

                <div>
                  <FldLabel required>{t('new.fields.state')}</FldLabel>
                ...
                <div>
                  <FldLabel required>{t('new.fields.line')}</FldLabel>
                ...
                <div>
                  <FldLabel>{t('new.fields.reason')}</FldLabel>
                  <textarea
                    value={form.reason}
                ...
                >
                  ...
                  />
                  <div>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: form.urgent ? '#C0392B' : '#181C23' }}>{t('new.urgent.title')}</div>
                    <div style={{ fontSize: 12, color: '#717786', marginTop: 2 }}>{t('new.urgent.desc')}</div>
                  </div>
                </label>
              </div>
            )}
            ...
            {step === 3 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <div style={{ marginBottom: 2 }}>
                  <h2 style={{ fontSize: 16, fontWeight: 800, color: '#181C23', margin: '0 0 6px' }}>{t('new.step3.title')}</h2>
                  <p style={{ fontSize: 13, color: '#717786' }}>{t('new.step3.subtitle')}</p>
                </div>
                ...
                <div style={{ borderRadius: 12, border: '0.5px solid rgba(0,88,188,0.2)', overflow: 'hidden' }}>
                  <div style={{ padding: '12px 18px', background: 'rgba(0,88,188,0.05)', borderBottom: '0.5px solid rgba(0,88,188,0.12)', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13.5, fontWeight: 700, color: '#181C23' }}>{t('new.summaryTitle')}</span>
                  </div>
                  <div style={{ padding: '4px 18px 10px' }}>
                    {[
                      [t('new.summary.channel'),   selectedChannel?.name ?? '—'],
                      [t('new.summary.channelNpn'), selectedChannel?.npn ?? '—'],
                      [t('new.summary.insurer'),   form.insurer || '—'],
                      [t('new.summary.state'),     form.state || '—'],
                      [t('new.summary.line'),      form.line || '—'],
                      [t('new.summary.priority'),  form.urgent ? t('new.priority.urgent') : t('new.priority.normal')],
                      ...(form.reason ? [[t('new.summary.reason'), form.reason]] as const : []),
                    ].map(([k, v]) => (
                      <div key={k} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '0.5px solid rgba(193,198,215,0.3)', padding: '10px 0', fontSize: 13, gap: 12 }}>
                        ...
                        <span style={{
                          fontWeight: 600,
                          color: k === t('new.summary.priority') && form.urgent ? '#C0392B' : '#181C23',
                          fontFamily: k === t('new.summary.channelNpn') ? "'JetBrains Mono', monospace" : undefined,
                          textAlign: 'right',
                        }}>{v}</span>
                      </div>
                    ))}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
            ...
            >
              <ArrowLeft size={14} />{step === 0 ? t('new.cancel') : t('new.prevStep')}
            </button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              >
                {step < STEPS.length - 1
                  ? <>{t('new.nextStep')} <ArrowLeft size={14} style={{ transform: 'rotate(180deg)' }} /></>
                  : saving
                    ? <><CheckCircle2 size={13} className="animate-spin" />{t('new.submitting')}</>
                    : <><CheckCircle2 size={13} />{t('new.confirmSubmit')}</>
                }
              </button>
            </div>
          </div>
