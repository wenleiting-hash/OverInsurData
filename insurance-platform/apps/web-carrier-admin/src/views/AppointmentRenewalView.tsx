...
export default function AppointmentRenewalView({ navigateTo }: Props) {
  ...
  const getUrgencyLabel = (daysRemaining: number): string => {
    if (daysRemaining <= 7) return 'CRITICAL - Renew Immediately';
    if (daysRemaining <= 30) return 'URGENT - Within 30 Days';
    if (daysRemaining <= 60) return 'WARNING - Within 60 Days';
    if (daysRemaining <= 90) return 'RENEWAL DUE SOON';
  ...
  const handleBatchRenew = () => {
    ...
    if (selected.length > 0) {
      alert(`Initiating batch renewal for ${selected.length} appointment(s)\n\nPlease review:\n1. Channel organization eligibility\n2. Outstanding compliance requirements\n3. State-specific filing fees`);
    }
  };
  
  const startRenewal = (app: AppointmentRecord) => {
    const daysUntilExp = getDaysUntilExpiration(app.expiryDate);
    alert(`Starting renewal process for:\n\nID: ${app.id}\nChannel: ${app.channelName}\nNPN: ${app.channelNpn}\nInsurer: ${app.insurerShort}\nState: ${app.state}\nLine of Authority: ${app.line}\n\nExpiring in: ${daysUntilExp} day(s)\n\nNext Steps:\n1. Verify current NIPR license status\n2. Submit renewal application via NIPR portal\n3. Pay state-specific filing fees\n4. Await carrier approval confirmation`);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-yellow-50 p-6">
      ...
      <div className="max-w-[1600px] mx-auto mb-6">
        ...
        <p className="text-gray-600 mt-1">Track and manage expiring NIPR appointment authorizations</p>
      </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-red-900 mb-1">
                    {stats.urgent30Days} Appointment(s) Expiring Within 30 Days - CRITICAL ACTION REQUIRED
                  </div>
                  <div className="text-sm text-red-800">
                    Immediate renewal submission needed to avoid coverage gaps and regulatory penalties
                  </div>
                </div>
                <button 
      ...
      <div className="max-w-[1600px] mx-auto mb-6 grid grid-cols-5 gap-4">
        <StatCard label="Expiring (90D)" value={stats.totalExpiring90Days.toString()} icon={<Calendar className="text-yellow-600"/>} color="yellow"/>
        <StatCard label="Critical (≤30D)" value={stats.urgent30Days.toString()} warn={stats.urgent30Days > 0} icon={<AlertTriangle className="text-red-600"/>} color="red"/>
        <StatCard label="In Progress" value={stats.inProgress.toString()} icon={<Clock className="text-blue-600"/>} color="blue"/>
        <StatCard label="Already Renewed" value={stats.renewed.toString()} icon={<CheckCircle className="text-green-600"/>} color="green"/>
        <StatCard label="Mild Warning (61-90D)" value={stats.mildWarning90Days.toString()} icon={<Filter className="text-gray-600"/>} color="gray"/>
      </div>
      ...
      <div className="max-w-[1600px] mx-auto card overflow-hidden">
        <div className="px-6 py-4 border-b bg-gray-50">
          <h3 className="font-semibold text-gray-900">Expiring Appointments List ({filteredApps.length} entries)</h3>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Insurer & State</th>
            ...
            {filteredApps.length === 0 && (
              <tr>
                <td colSpan={7} className="px-6 py-12 text-center text-gray-500">
                  ...
                  <div className="text-lg font-medium">No appointments expiring within 90 days</div>
                  <div className="text-sm mt-1">All current appointments are valid with sufficient time remaining</div>
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="max-w-[1600px] mx-auto mt-6 card">
        <div className="px-6 py-4 border-b">
          <h3 className="font-semibold text-gray-900">Renewal Process Guidelines</h3>
        </div>
        <div className="p-6 grid grid-cols-3 gap-6 text-sm text-gray-700">
          <div className="space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-600">
              <li>Confirm active NIPR license status</li>
              <li>Check CE requirement completion</li>
              <li>Verify no outstanding violations</li>
            </ul>
          </div>
          ...
          <div className="space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-600">
              <li>Login to NIPR portal</li>
              <li>Complete renewal form (AF-100)</li>
              <li>Pay state filing fees ($50-$200)</li>
          ...
          <div className="space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-600">
              <li>Awaiting carrier approval (7-14 days)</li>
              <li>NIPR processing (5-7 business days)</li>
              <li>Receive authorization certificate</li>
              <li>Update internal system records</li>
            </ul>
          </div>
        </div>
      </div>
