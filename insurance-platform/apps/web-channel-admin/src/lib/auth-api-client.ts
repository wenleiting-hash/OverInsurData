...
authApiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    // The /sso/exchange endpoint returns 401 as a BUSINESS error (invalid code / user not
    // synced) before any session exists. Auto-refresh would hijack it into a /login redirect,
    // bypassing SsoCallbackView's own error mapping. Let the caller handle it.
    const isSsoExchange = (originalRequest?.url || '').includes('/sso/exchange');

    // If 401 Unauthorized and not already retrying and not the refresh call itself
    if (error.response?.status === 401 && !originalRequest._retry && !isRefreshRequest && !isSsoExchange) {
