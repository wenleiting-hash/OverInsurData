/**
 * OverInsur (overinsur_db) Schema Definitions - V2.1 Design
 * 
 * Database: overinsur_db within OVERINSURDATA container (port 5433)
 * All tables follow V2.1 design without 'ovwr_' prefix for clarity
 */

import { pgTable, varchar, timestamp, index, jsonb, integer, text, boolean } from 'drizzle-orm/pg-core';
...
export const authUser = pgTable('auth_user', {
  ...
  idxEmail: index('idx_auth_user_email').on(table.email),
  idxUsername: index('idx_auth_user_username').on(table.username),
}));
