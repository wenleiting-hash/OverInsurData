...
import CommissionRateView from './views/CommissionRateView'
...
export default function App() {
  ...
  const [financeCommissionInsurerId, setFinanceCommissionInsurerId] = useState<string | undefined>(undefined)

  const navigateTo = (view: ViewId, params?: { insurerId?: string; productId?: string; commissionInsurerId?: string }) => {
    if (params?.insurerId) setSelectedInsurerId(params.insurerId)
    if (params?.productId) setSelectedProductId(params.productId)
    if (params?.commissionInsurerId) setFinanceCommissionInsurerId(params.commissionInsurerId)
    else if (view !== 'finance') setFinanceCommissionInsurerId(undefined)
    setCurrentView(view)
    window.scrollTo(0, 0)
  }
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'finance':
        return <FinanceView navigateTo={navigateTo} commissionInsurerId={financeCommissionInsurerId} />
      case 'insurer-analytics':
        return <InsurerAnalyticsView navigateTo={navigateTo} />
      // Ch4 合规管理
      case 'appointment-application':
      ...
      case 'ofac-screening':
        return <PlaceholderView viewId={currentView} navigateTo={navigateTo} />
      case 'channel-list':
