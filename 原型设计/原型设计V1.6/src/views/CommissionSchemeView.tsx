...
const SCHEME_STATUS: Record<SchemeStatus, { label: string; color: string; bg: string }> = {
  draft:    { label: '草稿', color: C.muted, bg: 'rgba(193,198,215,0.18)' },
  pending:  { label: '待审批', color: C.primary, bg: C.primaryLight },
  active:   { label: '已生效', color: C.green, bg: C.greenBg },
  inactive: { label: '已停用', color: C.red, bg: C.redBg },
  expired:  { label: '已失效', color: C.mutedLight, bg: 'rgba(193,198,215,0.12)' },
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
        {[
          { label: '生效方案数', v: activeCount, color: C.green, icon: <CheckCircle2 size={16} color={C.green} /> },
          { label: '待审批', v: pendingCount, color: C.primary, icon: <Clock size={16} color={C.primary} /> },
          { label: '绑定渠道数', v: totalBound, color: C.text, icon: <Layers size={16} color={C.primary} /> },
        ].map(s => (
        ...
        <GCard style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
          <div style={{ display: 'flex', gap: 8, padding: '10px 14px', borderBottom: `0.5px solid ${C.border}`, background: C.bg, flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: 1 }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索方案名称或编码…" style={{ width: '100%', padding: '6px 10px 6px 27px', background: 'rgba(255,255,255,0.5)', border: `0.5px solid ${C.border}`, borderRadius: 8, fontSize: 12.5, color: C.text, outline: 'none', fontFamily: 'inherit' }} />
            </div>
            {[
              { value: statusF, set: setStatusF, opts: [['all','全部状态'], ...Object.entries(SCHEME_STATUS).map(([k,v]) => [k, v.label])] },
              { value: lineF, set: setLineF, opts: [['all','全部业务线'], ...['AUTO','HOME','LIFE','HEALTH','COMMERCIAL'].map(l => [l, l])] },
            ].map((sel, i) => (
            ...
            <PrimaryBtn onClick={() => setShowForm(true)} sm><Plus size={13} />新建方案</PrimaryBtn>
          </div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['方案名称', '适用范围', '首年佣金率', '续期费率', '状态', '功能', '绑定数', '版本', ''].map(h => <th key={h} style={TH}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {filtered.map(s => {
                  ...
                  return (
                    <tr key={s.id} onClick={() => setSelected(isSel ? null : s)} style={{ background: isSel ? C.primaryLight : 'transparent', cursor: 'pointer' }}>
                      ...
                      <td style={TD}>
                        <div style={{ fontSize: 12, color: C.textSoft }}>{s.channelType}</div>
                      ...
                      <td style={TD}>
                        <div style={{ display: 'flex', gap: 7 }}>
                          <Feature on={s.ladderEnabled} label="阶梯" />
                          <Feature on={s.overrideEnabled} label="Override" />
                          <Feature on={s.chargebackEnabled} label="追回" />
                        </div>
                      </td>
          ...
          <GCard style={{ width: 300, flexShrink: 0, position: 'sticky', top: 0, maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '12px 18px' }}>
              ...
              <div style={{ marginBottom: 16, padding: '12px', borderRadius: 10, background: C.primaryLight, border: `0.5px solid ${C.primaryBorder}` }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.primary, marginBottom: 8 }}>年度佣金率结构</div>
              ...
              {[
                ['渠道类型', selected.channelType],
                ['保险公司', selected.insurer],
                ['业务线', selected.line],
                ['生效日期', selected.effectDate],
                ['佣金下限', selected.floorAmt ? `$${selected.floorAmt}` : '无'],
                ['佣金上限', selected.capAmt ? `$${selected.capAmt.toLocaleString()}` : '无'],
                ['创建人', selected.createdBy],
                ['审批人', selected.approvedBy ?? '待审批'],
              ].map(([k, v]) => (
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
                <Feature on={selected.ladderEnabled} label="阶梯佣金" />
                <Feature on={selected.overrideEnabled} label="Override" />
                <Feature on={selected.chargebackEnabled} label="退保追回" />
              </div>

              <div style={{ display: 'flex', gap: 7, marginTop: 14 }}>
                <PrimaryBtn sm><Edit2 size={12} />编辑</PrimaryBtn>
                <GhostBtn sm onClick={() => setShowImpact(true)}><BarChart3 size={12} />影响分析</GhostBtn>
              </div>
            </div>
          </GCard>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <GCard style={{ padding: '18px 20px' }}>
          <div style={{ fontWeight: 800, fontSize: 14, color: C.text, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 7 }}>
            <Layers size={15} color={C.primary} />阶梯佣金规则配置
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 18 }}>
            <div>
              <div style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, marginBottom: 8 }}>阶梯模式</div>
              <div style={{ display: 'flex', gap: 6 }}>
                {[['full', '全额累进'], ['marginal', '超额累进']] .map(([v, l]) => (
            ...
            <div>
              <div style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, marginBottom: 8 }}>考核周期</div>
              <div style={{ display: 'flex', gap: 6 }}>
                {[['monthly','月度'],['quarterly','季度'],['annual','年度']].map(([v, l]) => (
          ...
          <div style={{ borderRadius: 10, overflow: 'hidden', border: `0.5px solid ${C.border}` }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['档位', '保费下限', '保费上限', '费率系数', '等效费率', '操作'].map(h => <th key={h} style={{ ...TH, fontSize: 11 }}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {tiers.map((t, i) => (
                  <tr key={t.id} style={{ background: simPremium >= t.minPremium && (t.maxPremium === null || simPremium < t.maxPremium) ? C.primaryLight : 'transparent' }}>
                    <td style={{ ...TD, fontWeight: 700, color: C.text }}>第 {i + 1} 档</td>
                    <td style={{ ...TD, ...mono, fontSize: 12 }}>${t.minPremium.toLocaleString()}</td>
                    <td style={{ ...TD, ...mono, fontSize: 12 }}>{t.maxPremium ? `$${t.maxPremium.toLocaleString()}` : '不设上限'}</td>
      ...
      <div style={{ width: 280, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <GCard style={{ padding: '18px' }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: C.text, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
            <Zap size={13} color={C.primary} />阶梯模拟计算
          </div>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11.5, color: C.muted, marginBottom: 5 }}>模拟保费（$）</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { label: '命中档位', value: `第 ${tiers.findIndex(t => simPremium >= t.minPremium && (t.maxPremium === null || simPremium < t.maxPremium)) + 1} 档`, color: C.primary },
              { label: '费率系数', value: `×${multiplier.toFixed(2)}`, color: C.primary },
              { label: '实际佣金率', value: `${(baseRate * multiplier * 100).toFixed(1)}%`, color: C.green },
              { label: '预计佣金', value: `$${simCommission.toLocaleString(undefined, { maximumFractionDigits: 0 })}`, color: C.green },
            ].map(r => (
        ...
        <GCard style={{ padding: '16px 18px' }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: C.text, marginBottom: 12 }}>Override 层级配置</div>
          {overrideTiers.map(t => (
            <div key={t.level} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 0', borderBottom: `0.5px solid ${C.border}` }}>
              <div>
                <div style={{ fontSize: 12.5, fontWeight: 700, color: C.text }}>{t.role}</div>
                {t.minTeamSize && <div style={{ fontSize: 11, color: C.muted }}>最少 {t.minTeamSize} 人</div>}
              </div>
              <span style={{ ...mono, fontSize: 13, fontWeight: 800, color: C.amber }}>{(t.overrideRate * 100).toFixed(1)}%</span>
            </div>
          ))}
          <button style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 8, padding: '5px 10px', borderRadius: 7, fontSize: 12, color: C.primary, background: C.primaryLight, border: `0.5px solid ${C.primaryBorder}`, cursor: 'pointer', fontWeight: 600 }}><Plus size={11} />新增层级</button>
        </GCard>
      </div>
    </div>
  )
    ...
    <GCard style={{ overflow: 'hidden' }}>
      <div style={{ padding: '12px 14px', borderBottom: `0.5px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontWeight: 700, fontSize: 13.5, color: C.text, display: 'flex', alignItems: 'center', gap: 7 }}>
          <Clock size={14} color={C.primary} />待审批方案
        ...
        <GhostBtn sm><Download size={12} />批量审批记录</GhostBtn>
      </div>
      {pending.length === 0 ? (
          ...
          <div key={s.id} style={{ padding: '16px 18px', borderBottom: `0.5px solid ${C.border}` }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
              <div>
                ...
                <div style={{ ...mono, fontSize: 11, color: C.muted, marginTop: 2 }}>{s.code} · v{s.version} · {s.channelType} · {s.line}</div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 14, padding: '10px 12px', borderRadius: 9, background: C.bg }}>
              <div style={{ textAlign: 'center' as const }}>
                ...
                <div style={{ fontSize: 11, color: C.muted }}>首年佣金率</div>
              </div>
              <div style={{ textAlign: 'center' as const }}>
                <div style={{ ...mono, fontSize: 20, fontWeight: 800, color: C.textSoft }}>{(s.renewalRates[0] * 100).toFixed(0)}%</div>
                <div style={{ fontSize: 11, color: C.muted }}>续期佣金率Y2</div>
              </div>
              <div style={{ textAlign: 'center' as const }}>
                <div style={{ ...mono, fontSize: 20, fontWeight: 800, color: C.textSoft }}>{s.bindCount}</div>
                <div style={{ fontSize: 11, color: C.muted }}>绑定渠道数</div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <PrimaryBtn sm><Check size={12} />审批通过</PrimaryBtn>
              <GhostBtn sm><X size={12} />驳回</GhostBtn>
              <GhostBtn sm><Eye size={12} />查看详情</GhostBtn>
              <GhostBtn sm><BarChart3 size={12} />影响分析</GhostBtn>
            </div>
          </div>
        ))
      )}
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(24,28,35,0.45)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <GCard style={{ width: 620, maxHeight: '90vh', overflowY: 'auto', background: C.surfaceHigh, borderRadius: 18, padding: '28px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            <h2 style={{ fontSize: 17, fontWeight: 800, color: C.text, margin: 0 }}>新建佣金方案</h2>
            <p style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>步骤 {step} / 3</p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted }}><X size={16} /></button>
        </div>

        <div style={{ display: 'flex', gap: 4, marginBottom: 24 }}>
          {['基本信息', '费率配置', '附加规则'].map((s, i) => (
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ padding: '12px 14px', borderRadius: 10, background: C.primaryLight, border: `0.5px solid ${C.primaryBorder}` }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: C.primary, marginBottom: 10 }}>基本佣金率设置</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
                {['首年佣金率 *', '第2年续期率', '第3年续期率', '第4年续期率', '第5年续期率', '5年后续期率'].map(l => (
                  <div key={l}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 5 }}>佣金下限（$）</label>
                <input type="number" placeholder="可选" style={{ width: '100%', padding: '8px 11px', borderRadius: 8, border: `0.5px solid ${C.border}`, fontSize: 13, background: 'rgba(255,255,255,0.65)', fontFamily: 'JetBrains Mono, monospace', outline: 'none' }} />
              </div>
              <div>
                <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 5 }}>佣金上限（$）</label>
                <input type="number" placeholder="可选" style={{ width: '100%', padding: '8px 11px', borderRadius: 8, border: `0.5px solid ${C.border}`, fontSize: 13, background: 'rgba(255,255,255,0.65)', fontFamily: 'JetBrains Mono, monospace', outline: 'none' }} />
              </div>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24 }}>
          ...
          {step < 3
            ...
            : <PrimaryBtn onClick={onClose}><Check size={13} />提交审批</PrimaryBtn>
          }
        </div>
      </GCard>
    </div>
      ...
      <GCard style={{ width: 580, background: C.surfaceHigh, borderRadius: 18, padding: '28px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
          <h2 style={{ fontSize: 16, fontWeight: 800, color: C.text, margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            <BarChart3 size={17} color={C.primary} />方案影响分析
          </h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted }}><X size={16} /></button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 18 }}>
          {[['原方案', 'SCH-AUTO-000 v2 (已失效)'], ['新方案', 'SCH-AUTO-001 v3 (当前)'], ['分析范围', '全部绑定渠道（18个）'], ['时间范围', '过去12个月']].map(([k, v]) => (
            <div key={k}>
        ...
        {!done && !running && <PrimaryBtn onClick={run}><Zap size={13} />执行模拟计算</PrimaryBtn>}
        {running && (
          <div style={{ textAlign: 'center', padding: '32px 0', color: C.muted, fontSize: 13 }}>
            <div style={{ marginBottom: 8 }}>模拟计算中，正在重新计算 12 个月 · 18 个渠道的佣金数据…</div>
        ...
        {done && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
              {[
                { label: '原方案总佣金', value: '$2.84M', color: C.muted },
                { label: '新方案总佣金', value: '$3.19M', color: C.primary },
                { label: '差异', value: '+$350K (+12.3%)', color: C.green },
              ].map(s => (
            ...
            <div style={{ display: 'flex', gap: 7 }}>
              <GhostBtn sm><Download size={12} />导出分析报告</GhostBtn>
              <GhostBtn sm onClick={onClose}>关闭</GhostBtn>
            </div>
          </div>
        )}
      </GCard>
