...
const courses = [
  { id: 1, name: '保险基础知识 — 财产险', category: '合规必修', duration: 180, enrolled: 312, completed: 288, passRate: 0.92, required: true, status: 'published', expires: '2026-12-31', level: '初级' },
  { id: 2, name: '汽车险核保实务', category: '产品培训', duration: 120, enrolled: 198, completed: 165, passRate: 0.88, required: false, status: 'published', expires: '2026-09-30', level: '中级' },
  { id: 3, name: 'AML 反洗钱合规培训 2026', category: '合规必修', duration: 90, enrolled: 420, completed: 380, passRate: 0.95, required: true, status: 'published', expires: '2026-12-31', level: '初级' },
  { id: 4, name: '商业财产险高级销售技巧', category: '销售提升', duration: 240, enrolled: 88, completed: 42, passRate: 0.76, required: false, status: 'published', expires: null, level: '高级' },
  { id: 5, name: 'E&O 专业责任险介绍', category: '产品培训', duration: 60, enrolled: 145, completed: 98, passRate: 0.84, required: false, status: 'draft', expires: null, level: '中级' },
  { id: 6, name: '续保与客户留存策略', category: '销售提升', duration: 150, enrolled: 0, completed: 0, passRate: 0, required: false, status: 'draft', expires: null, level: '中级' },
]

const certifications = [
  { id: 1, name: '财产险基础认证 P-1', holder: 'Jennifer Walsh', issueDate: '2025-03-14', expiryDate: '2027-03-13', status: 'active', license: 'CA-INS-2023-7721', score: 96 },
  { id: 2, name: 'AML 合规认证', holder: 'Michael Torres', issueDate: '2025-06-01', expiryDate: '2026-05-31', status: 'expiring', license: 'CA-INS-2023-5512', score: 88 },
  { id: 3, name: '汽车险核保认证 U-2', holder: 'Amy Park', issueDate: '2025-01-20', expiryDate: '2026-01-19', status: 'expired', license: 'TX-INS-2022-3318', score: 84 },
  { id: 4, name: '财产险基础认证 P-1', holder: 'Lisa Wong', issueDate: '2026-04-10', expiryDate: '2028-04-09', status: 'active', license: 'CA-INS-2024-9901', score: 91 },
  { id: 5, name: 'E&O 专业责任险认证', holder: 'David Martinez', issueDate: '2025-08-15', expiryDate: '2027-08-14', status: 'active', license: 'CO-INS-2023-1144', score: 79 },
]

const exams = [
  { id: 1, course: '汽车险核保实务', date: '2026-09-05', candidates: 45, passed: 38, avgScore: 82.4, duration: 60, status: 'closed' },
  { id: 2, course: '保险基础知识 — 财产险', date: '2026-09-10', candidates: 28, passed: null, avgScore: null, duration: 90, status: 'upcoming' },
  { id: 3, course: 'AML 反洗钱合规培训 2026', date: '2026-08-28', candidates: 62, passed: 59, avgScore: 91.2, duration: 45, status: 'closed' },
]
...
function CourseLibraryTab() {
  ...
  const statusMeta: Record<string, { label: string; color: string; bg: string }> = {
    published: { label: '已发布', color: C.green, bg: C.greenBg },
    draft: { label: '草稿', color: C.amber, bg: C.amberBg },
  }
    ...
    <div style={{ display: 'flex', gap: 14, minHeight: 600 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
          {[
            { label: '课程总数', v: courses.length, color: C.primary },
            { label: '已发布', v: courses.filter(c => c.status === 'published').length, color: C.green },
            { label: '总报名人次', v: courses.reduce((s, c) => s + c.enrolled, 0), color: C.textSoft },
            { label: '平均通过率', v: `${(courses.filter(c=>c.passRate>0).reduce((s,c)=>s+c.passRate,0)/courses.filter(c=>c.passRate>0).length*100).toFixed(0)}%`, color: C.primary },
          ].map(k => (
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', background: 'rgba(255,255,255,0.4)', border: `0.5px solid ${C.border}`, borderRadius: 8, overflow: 'hidden' }}>
            {cats.map(c => (
              <button key={c} onClick={() => setFilter(c)} style={{ padding: '5px 12px', fontSize: 12, fontWeight: filter === c ? 700 : 500, background: filter === c ? C.primary : 'transparent', color: filter === c ? '#fff' : C.textSoft, border: 'none', cursor: 'pointer' }}>{c === 'all' ? '全部' : c}</button>
            ))}
          </div>
        ...
        <GCard style={{ overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>{['课程名称', '分类', '时长', '级别', '报名', '通过率', '状态', ''].map(h => <th key={h} style={TH}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} onClick={() => setSelected(c)} style={{ cursor: 'pointer', background: selected?.id === c.id ? C.primaryLight : 'transparent' }}>
                  <td style={TD}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 12.5, color: C.text }}>{c.name}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ ...TD, fontSize: 11.5, color: C.textSoft }}>{c.category}</td>
                  <td style={{ ...TD, ...mono, fontSize: 12, color: C.muted }}>{c.duration}分</td>
                  <td style={TD}><span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 5, background: 'rgba(107,53,194,0.09)', color: C.purple }}>{c.level}</span></td>
        ...
        <GCard style={{ width: 300, flexShrink: 0, alignSelf: 'flex-start', overflow: 'hidden' }}>
          <div style={{ padding: '14px 16px', borderBottom: `0.5px solid ${C.border}` }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 800, color: C.text }}>{selected.name}</div>
                <div style={{ fontSize: 11.5, color: C.muted, marginTop: 2 }}>{selected.category}</div>
              </div>
            </div>
          </div>
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { l: '级别', v: selected.level },
              { l: '时长', v: `${selected.duration} 分钟` },
              { l: '报名人数', v: String(selected.enrolled) },
              { l: '完成人数', v: String(selected.completed) },
              { l: '通过率', v: selected.passRate > 0 ? `${(selected.passRate * 100).toFixed(0)}%` : '暂无' },
              { l: '必修', v: selected.required ? '是' : '否' },
              { l: '有效期至', v: selected.expires ?? '长期有效' },
            ].map(r => (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 9, fontSize: 12.5, fontWeight: 700, background: C.primary, color: '#fff', border: 'none', cursor: 'pointer' }}>
          <ClipboardCheck size={13} />新建考试
        </button>
      </div>
      <GCard style={{ overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', borderBottom: `0.5px solid ${C.border}`, fontWeight: 700, fontSize: 13, color: C.text }}>考试管理</div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>{['课程名称', '考试日期', '时长', '参考人数', '通过人数', '平均分', '状态', ''].map(h => <th key={h} style={TH}>{h}</th>)}</tr></thead>
          <tbody>
            {exams.map(e => (
              <tr key={e.id}>
                <td style={TD}><div style={{ fontWeight: 700, fontSize: 12.5, color: C.text }}>{e.course}</div></td>
                <td style={{ ...TD, ...mono, fontSize: 12 }}>{e.date}</td>
                <td style={{ ...TD, ...mono, fontSize: 12, color: C.muted }}>{e.duration}分</td>
                ...
                <td style={TD}>
                  <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 6, fontWeight: 700, background: e.status === 'closed' ? 'rgba(193,198,215,0.2)' : C.primaryLight, color: e.status === 'closed' ? C.muted : C.primary }}>
                    {e.status === 'closed' ? '已结束' : '即将开始'}
                  </span>
                </td>
                <td style={TD}>
                  <div style={{ display: 'flex', gap: 5 }}>
                    <button style={{ padding: '3px 8px', borderRadius: 6, fontSize: 11, background: C.primaryLight, color: C.primary, border: `0.5px solid ${C.primaryBorder}`, cursor: 'pointer', fontWeight: 700 }}>报告</button>
                    <button style={{ padding: '3px 8px', borderRadius: 6, fontSize: 11, background: 'transparent', color: C.muted, border: `0.5px solid ${C.border}`, cursor: 'pointer' }}>详情</button>
                  </div>
                </td>
              </tr>
            ))}
...
function CertTab() {
  const statusMeta: Record<string, { label: string; color: string; bg: string; icon: React.ReactNode }> = {
    active: { label: '有效', color: C.green, bg: C.greenBg, icon: <CheckCircle2 size={12} color={C.green} /> },
    expiring: { label: '即将到期', color: C.amber, bg: C.amberBg, icon: <Clock size={12} color={C.amber} /> },
    expired: { label: '已过期', color: C.red, bg: C.redBg, icon: <XCircle size={12} color={C.red} /> },
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
        {[
          { label: '有效证书', v: certifications.filter(c => c.status === 'active').length, color: C.green, icon: <CheckCircle2 size={15} color={C.green} /> },
          { label: '即将到期 (30天内)', v: certifications.filter(c => c.status === 'expiring').length, color: C.amber, icon: <AlertTriangle size={15} color={C.amber} /> },
          { label: '已过期', v: certifications.filter(c => c.status === 'expired').length, color: C.red, icon: <XCircle size={15} color={C.red} /> },
        ].map(s => (
      ...
      <GCard style={{ overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', borderBottom: `0.5px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 13.5, color: C.text }}>认证证书管理</span>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>{['证书名称', '持证人', '执照号', '颁发日期', '到期日期', '成绩', '状态', ''].map(h => <th key={h} style={TH}>{h}</th>)}</tr></thead>
          <tbody>
            {certifications.map(c => {
              const m = statusMeta[c.status]
              return (
                <tr key={c.id} style={{ background: c.status === 'expiring' ? C.amberBg : c.status === 'expired' ? C.redBg : 'transparent' }}>
                  <td style={TD}><div style={{ fontWeight: 700, fontSize: 12.5, color: C.text }}>{c.name}</div></td>
                  ...
                  <td style={TD}>
                    <button style={{ padding: '3px 8px', borderRadius: 6, fontSize: 11, background: c.status === 'expiring' || c.status === 'expired' ? C.amberBg : C.primaryLight, color: c.status === 'expiring' || c.status === 'expired' ? C.amber : C.primary, border: `0.5px solid ${c.status === 'expiring' || c.status === 'expired' ? C.amberBorder : C.primaryBorder}`, cursor: 'pointer', fontWeight: 700 }}>
                      {c.status === 'expired' ? '续期' : c.status === 'expiring' ? '提醒' : '查看'}
                    </button>
                  </td>
                </tr>
              )
