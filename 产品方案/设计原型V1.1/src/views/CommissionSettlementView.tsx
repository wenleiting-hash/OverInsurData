...
const STATUS_META: Record<SettlStatus, { label: string; color: string; bg: string }> = {
  'pending-calc':     { label: '待计算', color: C.muted, bg: 'rgba(193,198,215,0.18)' },
  calculated:         { label: '已计算', color: C.primary, bg: C.primaryLight },
  'pending-approval': { label: '待审批', color: C.amber, bg: C.amberBg },
  approved:           { label: '已审批', color: C.green, bg: C.greenBg },
  paid:               { label: '已支付', color: C.green, bg: C.greenBg },
  failed:             { label: '支付失败', color: C.red, bg: C.redBg },
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {[
          { label: '本期应付佣金', v: fmt(periodData.reduce((a, s) => a + s.total, 0)), color: C.primary },
          { label: '待审批结算单', v: String(pendingApproval), color: C.amber },
          { label: '已审批待支付', v: fmt(periodData.filter(s => s.status === 'approved').reduce((a, s) => a + s.total, 0)), color: C.green },
          { label: '本期保单数', v: String(periodData.reduce((a, s) => a + s.policyCount, 0)), color: C.text },
        ].map(s => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 12.5, color: C.muted }}>结算周期</span>
        ...
        <PrimaryBtn onClick={runCalc} sm>
          {calcRunning ? <><Loader2 size={12} className="animate-spin" />计算中…</> : <><Zap size={12} />触发佣金计算</>}
        </PrimaryBtn>
        <GhostBtn sm><FileText size={12} />批量生成结算单</GhostBtn>
        <GhostBtn sm><Download size={12} />批量导出</GhostBtn>
      </div>
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        <GCard style={{ flex: 1, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['渠道', '类型', '基础佣金', 'Override', '奖金', '追回', '调整', '合计', '状态', '支付方式', ''].map(h => <th key={h} style={TH}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {periodData.map(s => {
                ...
                return (
                  <tr key={s.id} onClick={() => setSelected(isSel ? null : s)} style={{ background: isSel ? C.primaryLight : 'transparent', cursor: 'pointer' }}>
                    <td style={TD}>
                      ...
                      <div style={{ fontSize: 11, color: C.muted }}>{s.channelType}</div>
                    </td>
                    <td style={{ ...TD, fontSize: 11.5, color: C.muted }}>{s.policyCount} 保单</td>
                    ...
                    <td style={TD}>
                      <div style={{ display: 'flex', gap: 2 }}>
                        {s.status === 'pending-approval' && <button onClick={e => { e.stopPropagation() }} style={{ background: C.primary, border: 'none', cursor: 'pointer', color: '#fff', padding: '3px 8px', borderRadius: 5, fontSize: 11, fontWeight: 700 }}>审批</button>}
        ...
        {selected && (
          <GCard style={{ width: 300, flexShrink: 0, maxHeight: 'calc(100vh - 220px)', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ fontSize: 11.5, color: C.muted }}>{selected.period} · {selected.policyCount} 保单</div>

              {/* Breakdown chart */}
              <div style={{ borderRadius: 9, padding: '10px 12px', background: C.bg, marginTop: 4 }}>
                {[
                  { label: '基础佣金', v: selected.baseComm, color: C.primary },
                  { label: 'Override', v: selected.override, color: C.amber },
                  { label: '奖金', v: selected.bonus, color: C.green },
                  { label: '退保追回', v: selected.chargeback, color: C.red },
                  { label: '人工调整', v: selected.adjustment, color: selected.adjustment >= 0 ? C.green : C.red },
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0 0' }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>合计</span>
                  <span style={{ ...mono, fontSize: 14, fontWeight: 800, color: C.primary }}>{fmt(selected.total)}</span>
                </div>
              </div>

              {[
                ['支付方式', selected.payMethod.toUpperCase()],
                ['银行账户后4位', `****${selected.bankLast4}`],
                ...(selected.approvedBy ? [['审批人', selected.approvedBy]] : []),
                ...(selected.paidDate ? [['支付日期', selected.paidDate]] : []),
              ].map(([k, v]) => (
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                {selected.status === 'pending-approval' && <PrimaryBtn sm><Check size={12} />审批通过</PrimaryBtn>}
                {selected.status === 'approved' && <PrimaryBtn sm><Send size={12} />发起支付</PrimaryBtn>}
                <GhostBtn sm><Download size={12} />下载结算单</GhostBtn>
              </div>
            </div>
          </GCard>
        )}
...
function DetailTab() {
  ...
  const typeLabels: Record<string, string> = { direct: '直接佣金', override: 'Override', renewal: '续期佣金', bonus: '奖金', chargeback: '退保追回' }
    ...
    <GCard style={{ overflow: 'hidden' }}>
      <div style={{ display: 'flex', gap: 8, padding: '10px 14px', borderBottom: `0.5px solid ${C.border}`, background: C.bg }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索保单号或被保人…" style={{ width: '100%', padding: '6px 10px 6px 27px', background: 'rgba(255,255,255,0.5)', border: `0.5px solid ${C.border}`, borderRadius: 8, fontSize: 12.5, color: C.text, outline: 'none', fontFamily: 'inherit' }} />
        </div>
        ...
        <GhostBtn sm><Download size={12} />导出明细</GhostBtn>
      </div>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>{['保单号', '被保人', '保险公司 / 产品', '代理人', '佣金类型', '保费', '佣金率', '佣金金额', '状态'].map(h => <th key={h} style={TH}>{h}</th>)}</tr>
        </thead>
        <tbody>
          {filtered.map(d => (
            <tr key={d.id} style={{ background: d.commType === 'chargeback' ? C.redBg : 'transparent' }}>
              ...
              <td style={TD}><span style={{ fontSize: 11.5, color: d.status === '已结算' ? C.green : C.amber }}>{d.status}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <GCard style={{ padding: '16px 18px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: C.text }}>保险公司账单对账</div>
            <div style={{ fontSize: 12.5, color: C.muted, marginTop: 3 }}>Pacific Mutual · 2026-08</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <GhostBtn sm><Upload size={12} />上传账单文件</GhostBtn>
            <PrimaryBtn sm onClick={() => { setRunning(true); setTimeout(() => { setRunning(false) }, 2000) }}>
              {running ? <><Loader2 size={12} className="animate-spin" />对账中…</> : <><Zap size={12} />发起对账</>}
            </PrimaryBtn>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
          {[
            { label: '对账保单数', v: reconcileRows.length, color: C.text },
            { label: '一致', v: reconcileRows.filter(r => r.diffType === 'match').length, color: C.green },
            { label: '差异', v: diffRows.filter(r => !r.resolved).length, color: C.red },
            { label: '差异金额', v: `$${totalDiff.toFixed(2)}`, color: C.amber },
          ].map(s => (
      ...
      <GCard style={{ overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>{['保单号', '保险公司', '系统计算', '账单金额', '差异', '差异类型', '处理状态', '操作'].map(h => <th key={h} style={TH}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {reconcileRows.map(r => (
              <tr key={r.id} style={{ background: r.diffType !== 'match' && !r.resolved ? C.redBg : 'transparent' }}>
                ...
                <td style={TD}>
                  {r.diffType === 'match'
                    ? <span style={{ fontSize: 11, color: C.green, display: 'flex', alignItems: 'center', gap: 3 }}><CheckCircle2 size={11} />一致</span>
                    : <span style={{ padding: '2px 8px', borderRadius: 5, fontSize: 11, fontWeight: 700, background: `${diffTypeColor[r.diffType]}12`, color: diffTypeColor[r.diffType] }}>{DIFF_TYPE[r.diffType]}</span>
                  }
                </td>
                <td style={TD}>
                  {r.resolved ? <span style={{ fontSize: 11, color: C.green }}>已解决</span> : r.diffType !== 'match' ? <span style={{ fontSize: 11, color: C.amber }}>待处理</span> : null}
                </td>
                <td style={TD}>
                  {r.diffType !== 'match' && !r.resolved && (
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button style={{ background: C.greenBg, border: `0.5px solid ${C.greenBorder}`, cursor: 'pointer', color: C.green, padding: '3px 8px', borderRadius: 5, fontSize: 11, fontWeight: 700 }}>确认</button>
                      <button style={{ background: C.redBg, border: `0.5px solid ${C.redBorder}`, cursor: 'pointer', color: C.red, padding: '3px 8px', borderRadius: 5, fontSize: 11, fontWeight: 700 }}>申诉</button>
                    </div>
                  )}
                </td>
              </tr>
