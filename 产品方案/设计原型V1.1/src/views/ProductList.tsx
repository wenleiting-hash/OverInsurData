...
export default function ProductList({ navigateTo, onStatusChange }: Props) {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(new Set(['name', 'line', 'premium', 'lossRatio', 'renewalRate', 'status', 'actions']))
  ...
  return (
    <div style={{ maxWidth: 1440, margin: '0 auto' }}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 18px', borderTop: '0.5px solid rgba(193,198,215,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 12.5, color: '#717786' }}>共 {sorted.length} 条产品记录 · 每页显示
            <select className="input-glass" style={{ fontSize: 11.5, width: 60, marginLeft: 6, marginRight: 8 }}
          ...
          <div className="flex items-center gap-2">
            <button className="btn-ghost" style={{ padding: '4px 8px', fontSize: 12 }} disabled={currentPage === 1}
              onClick={() => setCurrentPage(d => d - 1)}>
              <ChevronLeft size={16} />
            </button>
            {[...Array(Math.ceil(sorted.length / pageSize))].map((_, i) => (
              <button key={i} className={currentPage === i + 1 ? 'btn-primary' : 'btn-ghost'} style={{ padding: '4px 10px', fontSize: 12 }}
                onClick={() => setCurrentPage(i + 1)}>{i + 1}</button>
            ))}
            <button className="btn-ghost" style={{ padding: '4px 8px', fontSize: 12 }} disabled={currentPage >= Math.ceil(sorted.length / pageSize)}
              onClick={() => setCurrentPage(d => d + 1)}>
