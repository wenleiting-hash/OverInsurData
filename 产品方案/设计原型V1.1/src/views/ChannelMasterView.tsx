...
function OrgFormModal({ mode, org, onClose }: { mode: 'create' | 'edit'; org?: ChannelOrg; onClose: () => void }) {
  ...
  const steps = ['基本信息', '总部 & 联系', '业务配置', '资质 & 提交']
  const STATES = ['CA','TX','NY','FL','IL','WA','OR','NV','CO','GA','OH','PA','NJ','CT','MA','AZ','NC','VA','TN','MO']
  const fld = (label: string, defaultValue: string, opts?: { span?: number; isMono?: boolean; required?: boolean; hint?: string }) => (
    <div key={label} style={{ gridColumn: opts?.span === 2 ? '1/-1' : undefined }}>
      <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'flex', alignItems: 'center', gap: 4, marginBottom: 5 }}>
        {label}{opts?.required && <span style={{ color: C.red }}>*</span>}
        {opts?.hint && <span style={{ fontWeight: 400, fontSize: 11, color: C.mutedLight }}>{opts.hint}</span>}
      </label>
      <input type="text" defaultValue={defaultValue} style={{ width: '100%', padding: '8px 11px', borderRadius: 8, border: `0.5px solid ${C.border}`, fontSize: 13, background: 'rgba(255,255,255,0.65)', fontFamily: 'inherit', outline: 'none', ...(opts?.isMono ? mono : {}) }} />
    </div>
  )
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(24,28,35,0.45)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <GlassCard style={{ width: 640, maxHeight: '90vh', overflowY: 'auto', background: C.surfaceHigh, borderRadius: 18, padding: '28px 32px', boxShadow: '0 24px 60px rgba(0,0,0,0.18)' }}>
        ...
        <div style={{ display: 'flex', marginBottom: 26, position: 'relative' }}>
          <div style={{ position: 'absolute', top: 11, left: '6.25%', right: '6.25%', height: 2, background: C.border, zIndex: 0 }}>
          ...
          {steps.map((s, i) => (
            <div key={s} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, position: 'relative', zIndex: 1 }}>
              <div style={{ width: 22, height: 22, borderRadius: '50%', background: i + 1 <= step ? C.primary : 'rgba(255,255,255,0.8)', border: `2px solid ${i + 1 <= step ? C.primary : C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                {i + 1 < step ? <Check size={11} color="#fff" /> : <span style={{ ...mono, fontSize: 10, fontWeight: 800, color: i + 1 === step ? '#fff' : C.mutedLight }}>{i + 1}</span>}
              </div>
              <span style={{ fontSize: 10.5, color: i + 1 <= step ? C.primary : C.mutedLight, fontWeight: i + 1 === step ? 700 : 400, whiteSpace: 'nowrap' }}>{s}</span>
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 6 }}>组织类型 <span style={{ color: C.red }}>*</span></label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {Object.entries(ORG_TYPE_LABEL).map(([k, v]) => {
                  const selected = (org?.type ?? 'agency') === k
                  const tc = ORG_TYPE_COLOR[k as keyof typeof ORG_TYPE_COLOR]
                  return (
                    <label key={k} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 8, cursor: 'pointer', fontSize: 12.5, fontWeight: selected ? 700 : 500, background: selected ? `${tc}12` : 'rgba(255,255,255,0.5)', color: selected ? tc : C.textSoft, border: `0.5px solid ${selected ? tc : C.border}` }}>
                      <input type="radio" name="orgType" defaultChecked={selected} style={{ display: 'none' }} />{v}
                    </label>
                  )
                })}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {fld('组织全称', org?.name ?? '', { span: 2, required: true })}
              {fld('简称', org?.shortName ?? '', { required: true })}
              {fld('品牌名称', '', { hint: '（对外展示用）' })}
              {fld('成立年份', '', { isMono: true })}
              {fld(t.formFieldNpn, org?.npn ?? '', { isMono: true, required: true })}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div>
                <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 5 }}>法律实体类型</label>
                <select defaultValue="LLC" style={{ width: '100%', padding: '8px 11px', borderRadius: 8, border: `0.5px solid ${C.border}`, fontSize: 13, background: 'rgba(255,255,255,0.65)', fontFamily: 'inherit', outline: 'none' }}>
                  {['LLC','Corporation (C-Corp)','Corporation (S-Corp)','Partnership','Sole Proprietorship','Non-Profit'].map(e => <option key={e}>{e}</option>)}
                </select>
              </div>
              {fld('EIN 税号', org?.taxId ?? '', { isMono: true })}
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {fld('主要联系人', org?.managerName ?? '', { span: 2 })}
            {fld('邮箱地址', org?.email ?? '', { required: true })}
            {fld('电话号码', org?.phone ?? '' )}
            {fld('官网', org?.website ?? '' )}
            <div>
              <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 5 }}>所在州 <span style={{ color: C.red }}>*</span></label>
              <select defaultValue={org?.state ?? 'CA'} style={{ width: '100%', padding: '8px 11px', borderRadius: 8, border: `0.5px solid ${C.border}`, fontSize: 13, background: 'rgba(255,255,255,0.65)', fontFamily: 'inherit', outline: 'none' }}>
                {STATES.map(s => <option key={s}>{s}</option>)}
              </select>
            </div>
            {fld('详细地址', org?.address ?? '', { span: 2 })}
            {fld('城市', org?.city ?? '' )}
            {fld('邮编', org?.zip ?? '', { isMono: true })}
            <div>
              <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 5 }}>上级机构（FMO / 总代）</label>
              <select defaultValue={org?.parentOrgId ?? ''} style={{ width: '100%', padding: '8px 11px', borderRadius: 8, border: `0.5px solid ${C.border}`, fontSize: 13, background: 'rgba(255,255,255,0.65)', fontFamily: 'inherit', outline: 'none' }}>
                <option value="">{t.formParentNone}</option>
                {channelOrgs.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
              </select>
            </div>
            {fld('合同编号', org?.contractId ?? '', { isMono: true })}
          </div>
        )}
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div>
                <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 5 }}>主营业务线</label>
              ...
              {fld('员工人数', '' )}
              {fld('代理人数（预估）', '' )}
              {fld('年保费规模预估 ($)', '', { isMono: true })}
            </div>

            <div>
              <label style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, display: 'block', marginBottom: 9 }}>授权经营州</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                {STATES.map(s => {
                  const on = (org?.licenseStates ?? ['CA']).includes(s)
                  return (
                    <label key={s} style={{ ...mono, display: 'flex', alignItems: 'center', padding: '4px 10px', borderRadius: 7, cursor: 'pointer', fontSize: 12.5, fontWeight: 700, background: on ? C.primaryLight : 'rgba(255,255,255,0.5)', color: on ? C.primary : C.textSoft, border: `0.5px solid ${on ? C.primaryBorder : C.border}` }}>
            ...
            <div style={{ borderRadius: 10, border: `0.5px solid ${C.border}`, overflow: 'hidden' }}>
              <div style={{ padding: '10px 14px', background: 'rgba(236,237,249,0.4)', borderBottom: `0.5px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: C.text }}>White-label 独立品牌门户</div>
                  <div style={{ fontSize: 11.5, color: C.muted, marginTop: 2 }}>为该渠道启用独立品牌的代理人门户</div>
                </div>
                <button onClick={() => setWhiteLabelOn(v => !v)} style={{ width: 40, height: 22, borderRadius: 11, background: whiteLabelOn ? C.primary : 'rgba(193,198,215,0.4)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' }}>
            ...
            <div>
              ...
              <textarea defaultValue={org?.notes ?? ''} rows={2} style={{ width: '100%', padding: '8px 11px', borderRadius: 8, border: `0.5px solid ${C.border}`, fontSize: 13, background: 'rgba(255,255,255,0.65)', fontFamily: 'inherit', resize: 'vertical', outline: 'none' }} />
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ fontSize: 12.5, color: C.muted, lineHeight: 1.6 }}>上传以下资质文件，确保合规性审核通过。所有文件审核通过后才可激活渠道。</div>
            {[
              ...
              <div key={doc.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.4)', border: `0.5px solid ${C.border}` }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {doc.required && <span style={{ fontSize: 10, color: C.red, fontWeight: 700 }}>必须</span>}
                  </div>
                  <div style={{ fontSize: 11.5, color: C.muted }}>{doc.hint}</div>
                </div>
                <button style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 8, fontSize: 12, fontWeight: 700, background: C.primaryLight, color: C.primary, border: `0.5px solid ${C.primaryBorder}`, cursor: 'pointer' }}>
                  <Upload size={11} />上传
                </button>
              </div>
            ))}
            <div style={{ padding: '10px 14px', borderRadius: 9, background: C.amberBg, border: `0.5px solid ${C.amberBorder}`, fontSize: 12.5, color: C.amber, display: 'flex', alignItems: 'center', gap: 7 }}>
              <AlertTriangle size={13} />提交后进入运营审核流程，通常1-3个工作日完成。审核期间渠道处于"待审核"状态。
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 24, paddingTop: 18, borderTop: `0.5px solid ${C.border}` }}>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {step === 4 && <GhostBtn onClick={() => doSave(true)}><FileText size={12} />保存草稿</GhostBtn>}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            {step < totalSteps
              ? <PrimaryBtn onClick={() => setStep(s => s + 1)}>{t.formNext}</PrimaryBtn>
              : <PrimaryBtn onClick={() => doSave(false)}>
                  {saving ? <><Loader2 size={13} className="animate-spin" />{t.formSaving}</> : <><Check size={13} />提交审核</>}
                </PrimaryBtn>
            }
          </div>
        </div>
        ...
        <GlassCard style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={{ ...TH, width: 38 }}><input type="checkbox" onChange={e => setCheckedIds(e.target.checked ? new Set(filtered.map(o => o.id)) : new Set())} checked={checkedIds.size === filtered.length && filtered.length > 0} /></th>
                  <th style={TH}>{t.colOrgName}</th>
                ...
                {filtered.map(org => {
                  ...
                  return (
                    <tr key={org.id} onClick={() => setSelected(isSel ? null : org)} style={{ background: isChecked ? 'rgba(0,88,188,0.05)' : isSel ? C.primaryLight : 'transparent', cursor: 'pointer', outline: isSel ? `1.5px solid ${C.primaryBorder}` : 'none', outlineOffset: -1 }}>
                      <td style={{ ...TD, width: 38 }} onClick={e => toggleCheck(org.id, e)}>
        ...
        <GlassCard style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={{ ...TH, width: 38 }}><input type="checkbox" onChange={e => setCheckedAgentIds(e.target.checked ? new Set(filtered.map(a => a.id)) : new Set())} checked={checkedAgentIds.size === filtered.length && filtered.length > 0} /></th>
                  <th style={TH}>{t.colAgent}</th>
                ...
                {filtered.map(agent => {
                  ...
                  return (
                    <tr key={agent.id} onClick={() => setSelected(isSel ? null : agent)} style={{ background: isChecked ? 'rgba(0,88,188,0.05)' : isSel ? C.primaryLight : 'transparent', cursor: 'pointer', outline: isSel ? `1.5px solid ${C.primaryBorder}` : 'none', outlineOffset: -1 }}>
                      <td style={{ ...TD, width: 38 }} onClick={e => toggleAgentCheck(agent.id, e)}>
                        <input type="checkbox" checked={isChecked} onChange={() => {}} style={{ cursor: 'pointer' }} />
                      </td>
                      <td style={TD}>
