...
import {
  ...
  PIPELINE_STEPS, STATUS_STYLE, CHANNEL_TYPE_LABEL, CHANNEL_TYPE_LABEL_EN, REVIEW_RESULT_STYLE,
...
const STEP_STATUS_LABEL: Record<string, { zh: string; en: string }> = {
  ...
  return (
    <div style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
      {app.steps.map((step, i) => {
        ...
        const stLabel = STEP_STATUS_LABEL[step.status]
        const title = lang === 'en'
          ? `${step.nameEn ?? step.name}: ${stLabel?.en ?? step.status}`
          : `${step.name}: ${stLabel?.zh ?? step.status}`
        return (
          <div key={step.id} title={title} style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <div key={step.id} style={{ display: 'flex', gap: 16, marginBottom: i < app.steps.length - 1 ? 14 : 0, position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ flex: 1, paddingTop: 4 }}>
              <div className="flex items-center gap-2">
                <span style={{ fontSize: 13, fontWeight: 600, color: '#181C23' }}>{lang === 'en' ? (step.nameEn ?? step.name) : step.name}</span>
                {step.status === 'in-progress' && <Badge bg="rgba(0,88,188,0.1)" color="#0058BC">{lang === 'en' ? 'In Progress' : '进行中'}</Badge>}
                {step.status === 'failed' && <Badge bg="rgba(255,59,48,0.1)" color="#C0392B">{lang === 'en' ? 'Failed' : '未通过'}</Badge>}
                {step.status === 'waived' && <Badge bg="rgba(180,180,180,0.15)" color="#717786">{lang === 'en' ? 'Waived' : '已豁免'}</Badge>}
              </div>
              {step.completedDate && <div style={{ fontSize: 11.5, color: '#A0A5B1', marginTop: 1 }}>{lang === 'en' ? `Completed ${step.completedDate}` : `完成于 ${step.completedDate}`}</div>}
              {step.notes && <div style={{ marginTop: 4, fontSize: 12, color: '#717786', background: 'rgba(255,255,255,0.6)', borderRadius: 7, padding: '5px 9px', border: '0.5px solid rgba(193,198,215,0.35)' }}>{lang === 'en' ? (step.notesEn ?? step.notes) : step.notes}</div>}
              {step.actionRequired && (
                <div style={{ marginTop: 5, fontSize: 12, color: '#C0392B', background: 'rgba(255,59,48,0.05)', borderRadius: 7, padding: '5px 9px', border: '1px solid rgba(255,59,48,0.2)', display: 'flex', gap: 6, alignItems: 'flex-start' }}>
                  <AlertTriangle size={12} style={{ flexShrink: 0, marginTop: 1 }} />{lang === 'en' ? (step.actionRequiredEn ?? step.actionRequired) : step.actionRequired}
                </div>
              )}
            </div>
          </div>
    ...
    <tr onClick={onClick} style={{ borderBottom: '0.5px solid rgba(193,198,215,0.25)', background: selected ? 'rgba(0,88,188,0.04)' : 'transparent', cursor: 'pointer', transition: 'background 0.1s' }}>
      <td style={{ padding: '12px 14px' }}>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: '#717786' }}>{lang === 'en' ? CHANNEL_TYPE_LABEL_EN[app.applicantType] : CHANNEL_TYPE_LABEL[app.applicantType]} · {app.state}</div>
          </div>
        </div>
      </td>
      <td style={{ padding: '12px 14px' }}><Badge bg={ss.bg} color={ss.color}>{lang === 'en' ? ss.labelEn : ss.label}</Badge></td>
      ...
      <td style={{ padding: '12px 14px' }}>
        {app.status === 'pending-supplement' && app.supplementDue && (
          <span style={{ fontSize: 11.5, fontWeight: 600, color: '#C0392B', display: 'flex', alignItems: 'center', gap: 3 }}><Clock size={11} />{lang === 'en' ? 'Due ' : '截止 '}{app.supplementDue}</span>
        )}
      </td>
    </tr>
  )
...
function ApplicationTab({ onSelectApp }: { onSelectApp: (app: OnboardingApp) => void }) {
  ...
  const quickStats = [
    { label: lang === 'en' ? 'Total Applications' : '总申请', value: onboardingStats.total, color: '#0058BC' },
    { label: lang === 'en' ? 'In Progress' : '进行中', value: onboardingStats.pending, color: '#B06000' },
    { label: lang === 'en' ? 'Onboarded' : '已入驻', value: onboardingStats.approved, color: '#1E8033' },
    { label: lang === 'en' ? 'Pending Supplement' : '待补充', value: onboardingStats.pendingSupplement, color: '#C0392B' },
    { label: lang === 'en' ? 'Avg Days' : '平均天数', value: onboardingStats.avgDays + 'd', color: '#7B3FCA' },
  ]
  ...
  const WIZARD_STEPS = lang === 'en'
    ? ['Basic Info', 'Qualifications', 'Channel Affiliation', 'Review & Submit']
    : ['基本信息', '资质材料', '渠道归属', '确认提交']
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder={lang === 'en' ? 'Search by applicant name or email…' : '搜索申请人姓名或邮箱…'} className="input-glass" style={{ paddingLeft: 30, width: '100%', fontSize: 12.5 }} />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as OnboardingStatus | 'all')} className="input-glass" style={{ fontSize: 12.5, minWidth: 130 }}>
            <option value="all">{lang === 'en' ? 'All Statuses' : '全部状态'}</option>
            {Object.entries(STATUS_STYLE).map(([k, v]) => <option key={k} value={k}>{lang === 'en' ? v.labelEn : v.label}</option>)}
          </select>
          <button onClick={() => setShowForm(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', borderRadius: 10, fontSize: 13, fontWeight: 700, background: '#0058BC', color: '#fff', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            <Plus size={14} />{lang === 'en' ? 'New Application' : '新增申请'}
          </button>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '0.5px solid rgba(193,198,215,0.5)', background: 'rgba(249,249,255,0.7)' }}>
                {(lang === 'en' ? ['Applicant', 'Status', 'Pipeline', 'Reviewer', 'Submitted', 'Due'] : ['申请人', '状态', '流程进度', '审核人', '提交日期', '截止']).map(h => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card style={{ padding: '16px 18px' }}>
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11.5, color: '#717786' }}>{lang === 'en' ? CHANNEL_TYPE_LABEL_EN[selected.applicantType] : CHANNEL_TYPE_LABEL[selected.applicantType]} · {selected.state}</div>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-1" style={{ fontSize: 12, marginBottom: 10 }}>
              {([
                ['email', lang === 'en' ? 'Email' : '邮箱', selected.email],
                ['phone', lang === 'en' ? 'Phone' : '电话', selected.phone],
                ['channel', lang === 'en' ? 'Parent Channel' : '归属渠道', selected.parentChannelName ?? '—'],
                ['reviewer', lang === 'en' ? 'Reviewer' : '审核人', selected.assignedReviewer ?? '—'],
                ['submitted', lang === 'en' ? 'Submitted' : '提交日期', selected.submittedDate],
                ...
                <div key={k} className="flex justify-between" style={{ padding: '4px 0', borderBottom: '0.5px solid rgba(193,198,215,0.2)' }}>
                  <span style={{ color: '#717786' }}>{label}</span>
            ...
            {selected.status === 'pending-supplement' && selected.rejectionReason && (
              <div style={{ padding: '10px 12px', borderRadius: 9, background: 'rgba(255,59,48,0.06)', border: '1px solid rgba(255,59,48,0.2)', fontSize: 12, color: '#7A2020', marginBottom: 8 }}>
                <div className="flex items-center gap-1.5" style={{ fontWeight: 700, marginBottom: 3 }}><AlertTriangle size={12} />{lang === 'en' ? 'Supplemental Materials Required' : '需要补充材料'}</div>
                {lang === 'en' ? (selected.rejectionReasonEn ?? selected.rejectionReason) : selected.rejectionReason}
                {selected.supplementDue && <div style={{ marginTop: 4, fontWeight: 700 }}>{lang === 'en' ? `Due: ${selected.supplementDue}` : `截止日期：${selected.supplementDue}`}</div>}
              </div>
            )}
            {selected.status === 'rejected' && selected.rejectionReason && (
              <div style={{ padding: '10px 12px', borderRadius: 9, background: 'rgba(255,59,48,0.06)', border: '1px solid rgba(255,59,48,0.2)', fontSize: 12, color: '#7A2020' }}>
                <div className="flex items-center gap-1.5" style={{ fontWeight: 700, marginBottom: 3 }}><XCircle size={12} />{lang === 'en' ? 'Rejection Reason' : '驳回原因'}</div>
                {lang === 'en' ? (selected.rejectionReasonEn ?? selected.rejectionReason) : selected.rejectionReason}
              </div>
            )}
          </Card>
          ...
          <Card style={{ padding: '14px 16px' }}>
            <div style={{ fontSize: 11.5, fontWeight: 700, color: '#A0A5B1', letterSpacing: 0.5, marginBottom: 12, textTransform: 'uppercase' as const }}>{lang === 'en' ? 'Pipeline Progress' : '流程进度'}</div>
          ...
          <div className="flex flex-col gap-2">
            {selected.status === 'pending-supplement' && (
              <button style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px', borderRadius: 9, fontSize: 13, fontWeight: 700, background: '#0058BC', color: '#fff', border: 'none', cursor: 'pointer' }}>
                <Upload size={13} />{lang === 'en' ? 'Upload Supplemental Materials' : '上传补充材料'}
              </button>
            )}
            {['submitted', 'under-review'].includes(selected.status) && (
              <div className="flex gap-2">
                <button style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '7px', borderRadius: 9, fontSize: 12.5, fontWeight: 700, background: '#1E8033', color: '#fff', border: 'none', cursor: 'pointer' }}>
                  <Check size={12} />{lang === 'en' ? 'Approve Document Review' : '通过资料审核'}
                </button>
                <button style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '7px', borderRadius: 9, fontSize: 12.5, fontWeight: 700, background: 'rgba(255,59,48,0.1)', color: '#C0392B', border: '1px solid rgba(255,59,48,0.25)', cursor: 'pointer' }}>
                  <X size={12} />{lang === 'en' ? 'Request Supplement' : '驳回补充'}
                </button>
              </div>
            )}
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(24,28,35,0.55)', backdropFilter: 'blur(4px)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="glass-strong" style={{ borderRadius: 20, width: 580, padding: '28px 32px', maxHeight: '88vh', overflowY: 'auto' }}>
            <div className="flex items-center justify-between mb-5">
              <h2 style={{ fontSize: 17, fontWeight: 700, color: '#181C23' }}>{lang === 'en' ? 'New Onboarding Application' : '新增入驻申请'}</h2>
            ...
            {formStep === 1 && (
              <div className="flex flex-col gap-4">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 5 }}>{lang === 'en' ? 'Channel Type *' : '渠道类型 *'}</label>
                    <select className="input-glass" style={{ width: '100%', fontSize: 13 }}>
                      {Object.entries(CHANNEL_TYPE_LABEL).map(([k, v]) => <option key={k} value={k}>{lang === 'en' ? CHANNEL_TYPE_LABEL_EN[k as keyof typeof CHANNEL_TYPE_LABEL_EN] : v}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 5 }}>{lang === 'en' ? 'Primary State *' : '主营州 *'}</label>
                    <select className="input-glass" style={{ width: '100%', fontSize: 13 }}>
                ...
                {[
                  { key: 'name', label: lang === 'en' ? 'Name / Agency Name *' : '姓名 / 机构名称 *', placeholder: lang === 'en' ? 'Full name or registered agency name' : '申请人全名或注册机构名' },
                  { key: 'email', label: lang === 'en' ? 'Email *' : '邮箱 *', placeholder: 'name@company.com' },
                  { key: 'phone', label: lang === 'en' ? 'Phone *' : '电话 *', placeholder: '+1-xxx-xxx-xxxx' },
                  { key: 'npn', label: lang === 'en' ? 'NPN' : 'NPN 编号', placeholder: lang === 'en' ? 'Optional; verified via NIPR' : '可选，将通过 NIPR 验证' },
                ].map(f => (
                  <div key={f.key}>
            ...
            {formStep === 2 && (
              <div className="flex flex-col gap-3">
                <div style={{ fontSize: 12.5, color: '#717786', marginBottom: 4 }}>{lang === 'en' ? 'Please upload the following onboarding documents:' : '请上传以下入驻所需材料：'}</div>
                {(lang === 'en'
                  ? ['Insurance License', 'E&O Insurance Certificate', 'Background Check Consent Form', 'Government-issued ID', 'W-9 Tax Form']
                  : ['保险执照（License）', 'E&O 保险证书', '背景调查授权书（Consent Form）', '政府颁发身份证件', 'W-9 税务表格']
                ).map((doc, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 10, border: '1px dashed rgba(193,198,215,0.5)', background: 'rgba(255,255,255,0.4)', gap: 8 }}>
                    <div className="flex items-center gap-2">
                    ...
                    <button style={{ padding: '5px 12px', borderRadius: 7, fontSize: 12, fontWeight: 600, background: 'rgba(0,88,188,0.1)', color: '#0058BC', border: '1px solid rgba(0,88,188,0.2)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                      <Upload size={11} />{lang === 'en' ? 'Upload' : '上传'}
                    </button>
                  </div>
                ))}
              </div>
            ...
            {formStep === 3 && (
              <div className="flex flex-col gap-4">
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 5 }}>{lang === 'en' ? 'Parent Channel *' : '上级渠道（Parent Channel）*'}</label>
                  <select className="input-glass" style={{ width: '100%', fontSize: 13 }}>
                    <option value="">{lang === 'en' ? 'Select a parent channel…' : '请选择上级渠道…'}</option>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 5 }}>{lang === 'en' ? 'Assign Reviewer' : '分配审核人'}</label>
                  <select className="input-glass" style={{ width: '100%', fontSize: 13 }}>
                ...
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 5 }}>{lang === 'en' ? 'Contract Template' : '合同模板'}</label>
                  <select className="input-glass" style={{ width: '100%', fontSize: 13 }}>
            ...
            {formStep === 4 && (
              <div style={{ padding: '14px 16px', borderRadius: 12, background: 'rgba(0,88,188,0.04)', border: '1px solid rgba(0,88,188,0.15)', fontSize: 12.5 }}>
                <div style={{ fontWeight: 700, color: '#0058BC', marginBottom: 8 }}>{lang === 'en' ? 'Submit Confirmation' : '提交确认'}</div>
                <div className="flex flex-col gap-2">
                  {(lang === 'en'
                    ? [
                        ['type', 'Channel Type', 'Individual Agent'],
                        ['applicant', 'Applicant', '(name entered)'],
                        ['state', 'Primary State', 'CA'],
                        ['parent', 'Parent Channel', 'Pacific Coast Insurance Group'],
                        ['template', 'Contract Template', 'Producer Agreement v3.2-2026'],
                      ]
                    : [
                        ['type', '渠道类型', '个人代理 (Agent)'],
                        ['applicant', '申请人', '（填写的姓名）'],
                        ['state', '主营州', 'CA'],
                        ['parent', '上级渠道', 'Pacific Coast Insurance Group'],
                        ['template', '合同模板', 'Producer Agreement v3.2-2026'],
                      ] as const
                  ).map(([k, label, v]) => (
                ...
                <div style={{ marginTop: 10, padding: '8px 10px', borderRadius: 8, background: 'rgba(52,199,89,0.07)', border: '1px solid rgba(52,199,89,0.2)', fontSize: 12, color: '#1E5A26' }}>
                  {lang === 'en' ? 'After submission, the system automatically sends an onboarding invitation email, starts the 10-step onboarding pipeline, and notifies the assigned reviewer.' : '提交后系统将自动发送入驻邀请邮件，启动 10 步入驻流程并通知分配审核人。'}
                </div>
              </div>
            )}
            ...
            <div className="flex justify-between mt-6">
              {formStep > 1 ? (
                <button className="btn-ghost" style={{ padding: '8px 16px', fontSize: 13 }} onClick={() => setFormStep(f => f - 1)}>{lang === 'en' ? 'Back' : '上一步'}</button>
              ) : <div />}
              {formStep < 4 ? (
                <button style={{ padding: '8px 20px', borderRadius: 9, fontSize: 13, fontWeight: 700, background: '#0058BC', color: '#fff', border: 'none', cursor: 'pointer' }} onClick={() => setFormStep(f => f + 1)}>{lang === 'en' ? 'Next' : '下一步'}</button>
              ) : (
                <button style={{ padding: '8px 20px', borderRadius: 9, fontSize: 13, fontWeight: 700, background: '#1E8033', color: '#fff', border: 'none', cursor: 'pointer' }} onClick={() => { setShowForm(false); setFormStep(1) }}>
                  <span className="flex items-center gap-1.5"><Send size={13} />{lang === 'en' ? 'Submit Application' : '提交申请'}</span>
                </button>
              )}
            </div>
          </div>
...
function DocReviewTab() {
  ...
  const DOC_TYPE_LABEL_EN: Record<string, string> = {
    license: 'Insurance License', 'eo-cert': 'E&O Certificate', 'bg-auth': 'BG Check Consent', id: 'ID Document', w9: 'W-9 Form', contract: 'Contract Document', training: 'Training Certificate', other: 'Other',
  }
  const docTypeLabel = (t: string) => lang === 'en' ? (DOC_TYPE_LABEL_EN[t] ?? t) : DOC_TYPE_LABEL[t]

  return (
    <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 18 }}>
      {/* App selector */}
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#A0A5B1', marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>{lang === 'en' ? 'Select Application' : '选择申请'}</div>
        <div className="flex flex-col gap-2">
          {onboardingApps.filter(a => !['approved', 'rejected', 'draft'].includes(a.status)).map(a => {
            ...
            return (
              <div key={a.id} onClick={() => setSelectedAppId(a.id)} style={{ padding: '10px 12px', borderRadius: 10, border: selectedAppId === a.id ? '1.5px solid #0058BC' : '1px solid rgba(193,198,215,0.3)', background: selectedAppId === a.id ? 'rgba(0,88,188,0.05)' : 'rgba(255,255,255,0.45)', cursor: 'pointer' }}>
                ...
                <Badge bg={ss.bg} color={ss.color}>{lang === 'en' ? ss.labelEn : ss.label}</Badge>
              </div>
            )
          })}
        </div>
          ...
          <>
            <Card style={{ padding: '14px 16px', marginBottom: 14 }}>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#717786', marginTop: 2 }}>{lang === 'en' ? CHANNEL_TYPE_LABEL_EN[app.applicantType] : CHANNEL_TYPE_LABEL[app.applicantType]} · {app.state} · {app.email}</div>
                </div>
                <div className="flex gap-2">
                  <button style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 700, background: '#1E8033', color: '#fff', border: 'none', cursor: 'pointer' }}>
                    <Check size={12} />{lang === 'en' ? 'Approve Document Review' : '资料审核通过'}
                  </button>
                  <button style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 700, background: 'rgba(255,59,48,0.1)', color: '#C0392B', border: '1px solid rgba(255,59,48,0.25)', cursor: 'pointer' }}>
                    <AlertTriangle size={12} />{lang === 'en' ? 'Request Supplement' : '要求补充'}
                  </button>
                </div>
              </div>
            </Card>
            ...
            {review ? (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <Badge bg={`rgba(0,88,188,0.08)`} color="#4A6A9C">{docTypeLabel(doc.docType)}</Badge>
                            </div>
                            <div style={{ fontSize: 12, color: '#717786', marginTop: 2 }}>
                              {doc.uploadedDate} · {doc.fileSize}
                              {doc.expiryDate && <span style={{ marginLeft: 8, color: new Date(doc.expiryDate) < new Date() ? '#C0392B' : '#717786', fontWeight: 600 }}>{lang === 'en' ? 'Expires ' : '有效期至 '}{doc.expiryDate}</span>}
                            </div>
                            {doc.reviewNote && (
                              <div style={{ marginTop: 6, padding: '6px 10px', borderRadius: 7, background: 'rgba(255,59,48,0.06)', border: '1px solid rgba(255,59,48,0.15)', fontSize: 12, color: '#7A2020', display: 'flex', gap: 5 }}>
                                <AlertTriangle size={11} style={{ flexShrink: 0, marginTop: 1 }} />{lang === 'en' ? (doc.reviewNoteEn ?? doc.reviewNote) : doc.reviewNote}
                              </div>
                            )}
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge bg={rs.bg} color={rs.color}>{lang === 'en' ? rs.labelEn : rs.label}</Badge>
                          <button className="btn-ghost" style={{ padding: 5 }} title={lang === 'en' ? 'Preview' : '预览'}><Eye size={13} /></button>
                          <button className="btn-ghost" style={{ padding: 5 }} title={lang === 'en' ? 'Download' : '下载'}><Download size={13} /></button>
                          {doc.status === 'fail' && (
                            <button onClick={() => fileInputRef.current?.click()} style={{ padding: '4px 10px', borderRadius: 7, fontSize: 11.5, fontWeight: 700, background: 'rgba(0,88,188,0.1)', color: '#0058BC', border: '1px solid rgba(0,88,188,0.2)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                              <Upload size={11} />{lang === 'en' ? 'Re-upload' : '补充上传'}
                            </button>
                          )}
                          {doc.status === 'pending' && (
                            <div className="flex gap-1">
                              <button style={{ padding: '4px 10px', borderRadius: 7, fontSize: 11.5, fontWeight: 700, background: 'rgba(52,199,89,0.1)', color: '#1E8033', border: '1px solid rgba(52,199,89,0.3)', cursor: 'pointer' }}>{lang === 'en' ? 'Pass' : '通过'}</button>
                              <button style={{ padding: '4px 10px', borderRadius: 7, fontSize: 11.5, fontWeight: 700, background: 'rgba(255,59,48,0.08)', color: '#C0392B', border: '1px solid rgba(255,59,48,0.2)', cursor: 'pointer' }}>{lang === 'en' ? 'Return' : '退回'}</button>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <Card style={{ textAlign: 'center', color: '#A0A5B1', padding: '40px 20px' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 600 }}>{lang === 'en' ? 'No document records' : '暂无文件记录'}</div>
                <div style={{ fontSize: 12, marginTop: 4 }}>{lang === 'en' ? 'This applicant has not uploaded onboarding materials yet' : '该申请人尚未上传入驻材料'}</div>
              </Card>
            )}
          </>
        )}
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-5">
        {([['nipr', lang === 'en' ? 'NIPR License Verification' : 'NIPR 牌照验证'], ['bg', lang === 'en' ? 'Background Check' : '背景调查'], ['eo', lang === 'en' ? 'E&O Insurance Verification' : 'E&O 保险验证']] as const).map(([v, l]) => (
      ...
      {subTab === 'nipr' && (
        <div className="flex flex-col gap-4">
          <Card style={{ padding: '14px 16px', background: 'rgba(0,88,188,0.03)' }}>
            <div style={{ fontSize: 12.5, color: '#4A6A9C', marginBottom: 10, fontWeight: 600 }}>{lang === 'en' ? 'NIPR Real-time Verification' : 'NIPR 实时验证'}</div>
            <div className="flex gap-3">
              <input placeholder={lang === 'en' ? 'Enter NPN…' : '输入 NPN 编号…'} className="input-glass" style={{ flex: 1, fontSize: 13, fontFamily: "'JetBrains Mono', monospace" }} />
              ...
              <button onClick={() => triggerVerify('manual')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', borderRadius: 9, fontSize: 13, fontWeight: 700, background: '#0058BC', color: '#fff', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                {verifying === 'manual' ? <Loader2 size={13} className="animate-spin" /> : <Search size={13} />} {lang === 'en' ? 'NIPR Lookup' : 'NIPR 查询'}
              </button>
            </div>
            {verifyResult === 'manual' && (
              <div style={{ marginTop: 10, padding: '10px 12px', borderRadius: 9, background: 'rgba(52,199,89,0.07)', border: '1px solid rgba(52,199,89,0.2)' }}>
                <div className="flex items-center gap-2" style={{ marginBottom: 6 }}><CheckCircle2 size={14} color="#1E8033" /><span style={{ fontWeight: 700, color: '#1E8033', fontSize: 13 }}>{lang === 'en' ? 'NIPR Verification Passed' : 'NIPR 验证通过'}</span></div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, fontSize: 12 }}>
                  {(lang === 'en'
                    ? [['License No.', 'CA-0M12345'],['License Type', 'Life, Accident & Health'],['Issue Date', '2019-03-15'],['Expiry Date', '2027-06-30'],['Status', 'Active'],['Lines of Authority', 'Life, A&H, P&C']]
                    : [['执照号', 'CA-0M12345'],['执照类型', 'Life, Accident & Health'],['颁发日期', '2019-03-15'],['到期日期', '2027-06-30'],['状态', 'Active'],['授权险种', 'Life, A&H, P&C']] as const
                  ).map(([k,v]) => (
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: '#181C23', marginBottom: 4 }}>{lang === 'en' ? 'Applicant Verification Records' : '申请人验证记录'}</div>
          <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge bg={rs.bg} color={rs.color}>{lang === 'en' ? rs.labelEn : rs.label}</Badge>
                        {app && <Badge bg={STATUS_STYLE[app.status].bg} color={STATUS_STYLE[app.status].color}>{lang === 'en' ? STATUS_STYLE[app.status].labelEn : STATUS_STYLE[app.status].label}</Badge>}
                      </div>
                      <div className="flex items-center gap-4" style={{ fontSize: 12, color: '#717786' }}>
                        ...
                        <span>{lang === 'en' ? 'State: ' : '州: '}<strong style={{ color: '#181C23' }}>{nv.state}</strong></span>
                        {nv.licenseNumber && <span>{lang === 'en' ? 'License: ' : '执照号: '}<strong style={{ fontFamily: "'JetBrains Mono', monospace", color: '#181C23' }}>{nv.licenseNumber}</strong></span>}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      {nv.expiryDate && <span style={{ fontSize: 11, color: '#717786', fontFamily: "'JetBrains Mono', monospace" }}>{lang === 'en' ? 'Expires ' : '到期 '}{nv.expiryDate}</span>}
                      {nv.status === 'pending' && (
                        <button onClick={() => triggerVerify(nv.appId)} disabled={verifying === nv.appId} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 8, fontSize: 12.5, fontWeight: 700, background: '#0058BC', color: '#fff', border: 'none', cursor: verifying === nv.appId ? 'not-allowed' : 'pointer' }}>
                          ...
                          {verifying === nv.appId ? (lang === 'en' ? 'Verifying…' : '验证中…') : (lang === 'en' ? 'Run Verification' : '发起验证')}
                        </button>
                      )}
                    </div>
                  </div>
      ...
      {subTab === 'bg' && (
        <div className="flex flex-col gap-4">
          <div style={{ padding: '12px 14px', borderRadius: 10, background: 'rgba(123,63,202,0.05)', border: '1px solid rgba(123,63,202,0.15)', fontSize: 12.5, color: '#4A2E7A' }}>
            <div className="flex items-center gap-1.5" style={{ fontWeight: 600, marginBottom: 2 }}><Shield size={13} />{lang === 'en' ? 'Background Check Providers' : '背景调查服务商'}</div>
            {lang === 'en' ? 'Currently integrated: Checkr (primary), Sterling (backup). Checks cover: criminal records, regulatory records, employment history, and credit reports (requires the applicant’s written consent).' : '当前集成：Checkr（主要）、Sterling（备用）。调查范围包括：刑事记录、监管记录、雇佣历史、信用报告（需申请人书面授权）。'}
          </div>
                ...
                <Card key={bc.appId} style={{ padding: '14px 16px', border: bc.flags.length > 0 ? `1.5px solid rgba(255,59,48,0.3)` : '1px solid rgba(193,198,215,0.3)' }}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-1" style={{ fontSize: 12, color: '#717786' }}>
                        <span>{lang === 'en' ? `Ordered: ${bc.orderedDate}` : `下单日期：${bc.orderedDate}`}</span>
                        {bc.completedDate && <span>{lang === 'en' ? `Completed: ${bc.completedDate}` : `完成日期：${bc.completedDate}`}</span>}
                        {!bc.completedDate && <span className="flex items-center gap-1" style={{ color: '#B06000' }}><Clock size={11} />{lang === 'en' ? 'In progress' : '进行中'}</span>}
                      </div>
                    </div>
                    <div className="flex gap-2">
                      {bc.status === 'pending' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 7, background: 'rgba(255,159,10,0.1)', fontSize: 12, color: '#B06000' }}>
                          <Loader2 size={11} className="animate-spin" />{lang === 'en' ? 'Est. 1–2 business days' : '预计 1–2 个工作日'}
                        </div>
                      )}
                  ...
                  {bc.flags.length > 0 && (
                    <div className="flex flex-col gap-2 mt-2">
                      {bc.flags.map((f, i) => {
                        const sev = f.severity === 'high' ? { bg: 'rgba(255,59,48,0.1)', color: '#C0392B', label: lang === 'en' ? 'High Risk' : '高风险' } : f.severity === 'medium' ? { bg: 'rgba(255,159,10,0.1)', color: '#B06000', label: lang === 'en' ? 'Medium Risk' : '中风险' } : { bg: 'rgba(180,180,180,0.15)', color: '#717786', label: lang === 'en' ? 'Low Risk' : '低风险' }
                        return (
                          <div key={i} style={{ padding: '10px 12px', borderRadius: 9, background: sev.bg, border: `1px solid ${sev.color}20` }}>
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <Badge bg={f.disposition === 'disqualify' ? 'rgba(255,59,48,0.12)' : 'rgba(255,159,10,0.12)'} color={f.disposition === 'disqualify' ? '#C0392B' : '#B06000'}>
                                {f.disposition === 'disqualify' ? (lang === 'en' ? 'Disqualified' : '不予准入') : f.disposition === 'review' ? (lang === 'en' ? 'Needs Review' : '需复核') : (lang === 'en' ? 'No Impact' : '无影响')}
                              </Badge>
                            </div>
                            <div style={{ fontSize: 12.5, color: '#181C23' }}>{lang === 'en' ? (f.descriptionEn ?? f.description) : f.description}</div>
                          </div>
                        )
                      })}
                    </div>
                  ...
                  {bc.flags.length === 0 && bc.status === 'pass' && (
                    <div style={{ padding: '8px 10px', borderRadius: 8, background: 'rgba(52,199,89,0.07)', border: '1px solid rgba(52,199,89,0.2)', fontSize: 12, color: '#1E5A26', display: 'flex', alignItems: 'center', gap: 5 }}>
                      <CheckCircle2 size={12} />{lang === 'en' ? 'No violations found; background check passed' : '无违规记录，背景调查通过'}
                    </div>
                  )}
                </Card>
              )
      ...
      {subTab === 'eo' && (
        <div className="flex flex-col gap-4">
          <div style={{ padding: '12px 14px', borderRadius: 10, background: 'rgba(0,88,188,0.04)', border: '1px solid rgba(0,88,188,0.15)', fontSize: 12.5, color: '#3A5A8C' }}>
            <div className="flex items-center gap-1.5" style={{ fontWeight: 600, marginBottom: 2 }}><Shield size={13} />{lang === 'en' ? 'E&O Insurance Requirements' : 'E&O 保险要求'}</div>
            {lang === 'en' ? 'Minimum limits for individual agents: $500,000 per occurrence / $1,000,000 aggregate; for agencies: $1,000,000 per occurrence / $2,000,000 aggregate. The E&O certificate must name InsureOS as an Additional Insured.' : '个人代理最低限额：$500,000 per occurrence / $1,000,000 aggregate；机构代理最低限额：$1,000,000 per occurrence / $2,000,000 aggregate。E&O证书须注明 InsureOS 为附加被保险人（Additional Insured）。'}
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: '#717786', marginBottom: 6 }}>
                        {eo.carrier} · {lang === 'en' ? 'Policy ' : '保单号 '}<span style={{ fontFamily: "'JetBrains Mono', monospace", color: '#181C23', fontWeight: 600 }}>{eo.policyNumber}</span>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 6, fontSize: 12 }}>
                        <div><span style={{ color: '#717786' }}>{lang === 'en' ? 'Coverage (per claim): ' : '承保金额（per claim）：'}</span><strong style={{ fontFamily: "'JetBrains Mono', monospace", color: '#181C23' }}>{fmt(eo.perClaimAmount)}</strong></div>
                        <div><span style={{ color: '#717786' }}>{lang === 'en' ? 'Aggregate limit: ' : '总承保限额：'}</span><strong style={{ fontFamily: "'JetBrains Mono', monospace", color: '#181C23' }}>{fmt(eo.coverageAmount)}</strong></div>
                        <div><span style={{ color: '#717786' }}>{lang === 'en' ? 'Effective: ' : '生效日期：'}</span><strong style={{ fontFamily: "'JetBrains Mono', monospace", color: '#555' }}>{eo.effectiveDate}</strong></div>
                        <div><span style={{ color: '#717786' }}>{lang === 'en' ? 'Expiry: ' : '到期日期：'}</span><strong style={{ fontFamily: "'JetBrains Mono', monospace", color: isExpired ? '#C0392B' : '#555' }}>{eo.expiryDate}{isExpired ? (lang === 'en' ? ' ⚠ Expired' : ' ⚠ 已过期') : ''}</strong></div>
                      </div>
                      {eo.note && <div style={{ marginTop: 6, fontSize: 12, color: '#C0392B' }}>{lang === 'en' ? (eo.noteEn ?? eo.note) : eo.note}</div>}
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      {eo.verifiedDate && <span style={{ fontSize: 11, color: '#A0A5B1' }}>{lang === 'en' ? `Verified ${eo.verifiedDate}` : `验证于 ${eo.verifiedDate}`}</span>}
                      {eo.status === 'fail' && (
                        <button style={{ padding: '5px 12px', borderRadius: 7, fontSize: 12, fontWeight: 700, background: 'rgba(0,88,188,0.1)', color: '#0058BC', border: '1px solid rgba(0,88,188,0.2)', cursor: 'pointer' }}>{lang === 'en' ? 'Re-verify' : '重新验证'}</button>
                      )}
                    </div>
                  </div>
                </Card>
...
function ContractTab() {
  ...
  const statusStyle: Record<string, { label: string; bg: string; color: string }> = {
    'not-sent': { label: lang === 'en' ? 'Not Sent' : '未发送', bg: 'rgba(180,180,180,0.15)', color: '#717786' },
    sent:       { label: lang === 'en' ? 'Sent' : '已发送', bg: 'rgba(0,88,188,0.1)',    color: '#0058BC' },
    viewed:     { label: lang === 'en' ? 'Viewed' : '已查看', bg: 'rgba(255,159,10,0.1)',  color: '#B06000' },
    signed:     { label: lang === 'en' ? 'Signed' : '已签署', bg: 'rgba(52,199,89,0.1)',   color: '#1E8033' },
    expired:    { label: lang === 'en' ? 'Expired' : '已过期', bg: 'rgba(255,59,48,0.1)',   color: '#C0392B' },
    declined:   { label: lang === 'en' ? 'Declined' : '已拒绝', bg: 'rgba(255,59,48,0.1)',   color: '#C0392B' },
  }

  const contractTypeLabel: Record<string, string> = {
    'producer-agreement': lang === 'en' ? 'Producer Agreement' : '代理人协议', 'ga-agreement': lang === 'en' ? 'GA Agreement' : 'GA协议',
    'sub-producer': lang === 'en' ? 'Sub-producer Agreement' : '子代理协议', 'branch-appointment': lang === 'en' ? 'Branch Appointment' : '分支机构委任书',
  }
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 18 }}>
      <div>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#181C23', marginBottom: 12 }}>{lang === 'en' ? 'E-Contract Status' : '电子合同状态'}</div>
        <div className="flex flex-col gap-3">
          {allContracts.map(c => {
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: '#717786' }}>
                      {lang === 'en' ? `Template: ${c.templateVersion} · Signing platform: ${c.signingProvider}` : `模板版本：${c.templateVersion} · 签署平台：${c.signingProvider}`}
                    </div>
                    <div style={{ fontSize: 12, color: '#717786', marginTop: 2 }}>
                      {lang === 'en' ? `Email: ${c.signerEmail}` : `邮件地址：${c.signerEmail}`}
                    </div>

                    {c.status === 'viewed' && c.expiresAt && (
                      <div style={{ marginTop: 6, fontSize: 12, color: '#B06000', display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Clock size={11} />{lang === 'en' ? `Applicant has viewed the contract; awaiting signature · Due ${c.expiresAt}` : `申请人已查看合同，等待签署 · 截止日期 ${c.expiresAt}`}
                      </div>
                    )}
                    {c.status === 'signed' && c.signedDate && (
                      <div style={{ marginTop: 6, fontSize: 12, color: '#1E8033', display: 'flex', alignItems: 'center', gap: 4 }}>
                        <CheckCircle2 size={11} />{lang === 'en' ? `E-signed on ${c.signedDate}` : `已于 ${c.signedDate} 完成电子签署`}
                      </div>
                    )}
                    {(localStatus === 'sent') && (
                      <div style={{ marginTop: 6, fontSize: 12, color: '#0058BC', display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Send size={11} />{lang === 'en' ? `Signing link sent to ${c.signerEmail}` : `已发送签署链接至 ${c.signerEmail}`}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {['not-sent'].includes(localStatus) && (
                      <button onClick={() => doSend(c.appId)} disabled={sending === c.appId} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 700, background: '#0058BC', color: '#fff', border: 'none', cursor: 'pointer' }}>
                        ...
                        {sending === c.appId ? (lang === 'en' ? 'Sending…' : '发送中…') : (lang === 'en' ? 'Send Contract' : '发送合同')}
                      </button>
                    )}
                    {localStatus === 'sent' && (
                      <button className="btn-ghost" style={{ fontSize: 12, padding: '5px 10px', display: 'flex', alignItems: 'center', gap: 4 }}><RotateCcw size={11} />{lang === 'en' ? 'Resend' : '重新发送'}</button>
                    )}
                    {localStatus === 'viewed' && (
                      <button className="btn-ghost" style={{ fontSize: 12, padding: '5px 10px', display: 'flex', alignItems: 'center', gap: 4 }}><Send size={11} />{lang === 'en' ? 'Remind' : '催签'}</button>
                    )}
                    {['signed'].includes(localStatus) && (
                      <button className="btn-ghost" style={{ fontSize: 12, padding: '5px 10px', display: 'flex', alignItems: 'center', gap: 4 }}><Download size={11} />{lang === 'en' ? 'Download Contract' : '下载合同'}</button>
                    )}
                  </div>
                </div>
              </Card>
      ...
      {/* Template side panel */}
      <div>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#181C23', marginBottom: 12 }}>{lang === 'en' ? 'Contract Template Library' : '合同模板库'}</div>
        <div className="flex flex-col gap-3">
          {templates.map(t => (
            <Card key={t.id} style={{ padding: '12px 14px' }}>
              <div style={{ fontWeight: 700, fontSize: 13, color: '#181C23', marginBottom: 3 }}>{t.name}</div>
              <div style={{ fontSize: 11.5, color: '#717786' }}>{lang === 'en' ? 'Version: ' : '版本：'}{t.version}</div>
              <div style={{ fontSize: 11, color: '#A0A5B1', marginTop: 2 }}>{lang === 'en' ? 'Last updated: ' : '最后更新：'}{t.lastUpdated}</div>
              <div className="flex gap-2 mt-3">
                <button className="btn-ghost" style={{ flex: 1, padding: '5px 8px', fontSize: 11.5, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}><Eye size={11} />{lang === 'en' ? 'Preview' : '预览'}</button>
                <button className="btn-ghost" style={{ flex: 1, padding: '5px 8px', fontSize: 11.5, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}><Download size={11} />{lang === 'en' ? 'Download' : '下载'}</button>
              </div>
            </Card>
          ))}
          <button className="btn-ghost" style={{ padding: '8px', fontSize: 12.5, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            <Plus size={13} />{lang === 'en' ? 'Upload New Template' : '上传新模板'}
          </button>
        </div>
      </div>
    </div>
...
function AccountTrainingTab() {
  ...
  const accountStatusStyle: Record<string, { label: string; bg: string; color: string }> = {
    'not-started': { label: lang === 'en' ? 'Not Created' : '未创建', bg: 'rgba(180,180,180,0.15)', color: '#717786' },
    creating:      { label: lang === 'en' ? 'Creating' : '创建中', bg: 'rgba(255,159,10,0.1)',   color: '#B06000' },
    active:        { label: lang === 'en' ? 'Activated' : '已激活', bg: 'rgba(52,199,89,0.1)',    color: '#1E8033' },
    hold:          { label: lang === 'en' ? 'Frozen' : '已冻结', bg: 'rgba(255,59,48,0.1)',    color: '#C0392B' },
  }

  const trainingStatusStyle = {
    'not-started':  { label: lang === 'en' ? 'Not Started' : '未开始',   bg: 'rgba(180,180,180,0.15)', color: '#717786' },
    'in-progress':  { label: lang === 'en' ? 'In Progress' : '学习中',   bg: 'rgba(255,159,10,0.1)',   color: '#B06000' },
    completed:      { label: lang === 'en' ? 'Completed' : '已完成',   bg: 'rgba(52,199,89,0.1)',    color: '#1E8033' },
    failed:         { label: lang === 'en' ? 'Failed' : '未通过',   bg: 'rgba(255,59,48,0.1)',    color: '#C0392B' },
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-5">
        {([['account', lang === 'en' ? 'System Account Setup' : '系统账号开通'], ['training', lang === 'en' ? 'Training & Certification' : '培训认证']] as const).map(([v, l]) => (
          <button key={v} onClick={() => setSubTab(v)} style={{ padding: '7px 18px', borderRadius: 9, fontSize: 13, fontWeight: 600, border: subTab === v ? '1.5px solid #0058BC' : '1px solid rgba(193,198,215,0.4)', background: subTab === v ? 'rgba(0,88,188,0.1)' : 'rgba(255,255,255,0.5)', color: subTab === v ? '#0058BC' : '#717786', cursor: 'pointer' }}>{l}</button>
        ))}
      </div>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {acc.username ? (
                      <div className="flex items-center gap-4 mb-2" style={{ fontSize: 12 }}>
                        <span style={{ color: '#717786' }}>{lang === 'en' ? 'Username: ' : '用户名：'}<strong style={{ fontFamily: "'JetBrains Mono', monospace", color: '#181C23' }}>{acc.username}</strong></span>
                        <span style={{ color: '#717786' }}>{lang === 'en' ? 'Email: ' : '邮箱：'}<strong style={{ color: '#181C23' }}>{acc.email}</strong></span>
                        {acc.activatedDate && <span style={{ color: '#717786' }}>{lang === 'en' ? 'Activated: ' : '激活于：'}<strong style={{ color: '#1E8033' }}>{acc.activatedDate}</strong></span>}
                      </div>
                    ) : (
                      <div style={{ fontSize: 12, color: '#717786', marginBottom: 8 }}>{lang === 'en' ? 'Email: ' : '邮箱：'}{acc.email}</div>
                    )}
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {acc.status === 'not-started' && (
                      <button onClick={() => doActivate(acc.appId)} disabled={activating === acc.appId} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 700, background: '#0058BC', color: '#fff', border: 'none', cursor: 'pointer' }}>
                        ...
                        {activating === acc.appId ? (lang === 'en' ? 'Creating…' : '创建中…') : (lang === 'en' ? 'Create Account' : '创建账号')}
                      </button>
                    )}
                    {acc.status === 'active' && (
                      <div className="flex gap-2">
                        <button onClick={() => doSendPwd(acc.appId)} disabled={sendingPwd === acc.appId} className="btn-ghost" style={{ padding: '5px 10px', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
                          ...
                          {acc.tempPasswordSent ? (lang === 'en' ? 'Resend Password' : '重发密码') : (lang === 'en' ? 'Send Temp Password' : '发送临时密码')}
                        </button>
                        <button className="btn-ghost" style={{ padding: '5px 10px', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}><Settings size={11} />{lang === 'en' ? 'Permissions' : '权限管理'}</button>
                        <button className="btn-ghost" style={{ padding: '5px 10px', fontSize: 12, color: '#C0392B', display: 'flex', alignItems: 'center', gap: 4 }}><Lock size={11} />{lang === 'en' ? 'Freeze' : '冻结'}</button>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <Card key={cert.appId} style={{ padding: '16px 18px' }}>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge bg={rrs.bg} color={rrs.color}>{lang === 'en' ? rrs.labelEn : rrs.label}</Badge>
                    </div>
                    <div style={{ fontSize: 12, color: '#717786' }}>
                      {lang === 'en'
                        ? <>Required completed: {requiredDone}/{requiredTotal} · All completed: {completedCount}/{cert.modules.length}</>
                        : <>必修完成：{requiredDone}/{requiredTotal} · 全部完成：{completedCount}/{cert.modules.length}</>}
                      {cert.overallStatus === 'pass' && cert.completionDate && <span style={{ marginLeft: 8, color: '#1E8033', fontWeight: 600 }}>{lang === 'en' ? `Certified on ${cert.completionDate}` : `认证完成于 ${cert.completionDate}`}</span>}
                      {avgScore > 0 && <span style={{ marginLeft: 8 }}>{lang === 'en' ? 'Avg score: ' : '平均分：'}<strong style={{ fontFamily: "'JetBrains Mono', monospace", color: avgScore >= 90 ? '#1E8033' : avgScore >= 80 ? '#B06000' : '#C0392B' }}>{avgScore.toFixed(0)}</strong></span>}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {cert.certUrl && <button className="btn-ghost" style={{ padding: '5px 10px', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}><Download size={11} />{lang === 'en' ? 'Download Certificate' : '证书下载'}</button>}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, fontSize: 11.5, color: '#717786' }}>
                    <span>{lang === 'en' ? 'Training Progress' : '培训进度'}</span>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
                  <thead>
                    <tr style={{ borderBottom: '0.5px solid rgba(193,198,215,0.4)' }}>
                      {(lang === 'en' ? ['Course', 'Duration', 'Required', 'Status', 'Score', 'Completed'] : ['课程名称', '时长', '必修', '状态', '得分', '完成日期']).map(h => (
                      ...
                      return (
                        <tr key={m.id} style={{ borderBottom: i < cert.modules.length - 1 ? '0.5px solid rgba(193,198,215,0.2)' : 'none' }}>
                          <td style={{ padding: '8px 8px', fontWeight: 600, color: '#181C23' }}>
                            <div className="flex items-center gap-1.5">{lang === 'en' ? (m.nameEn ?? m.name) : m.name}</div>
                          </td>
                          <td style={{ padding: '8px 8px', color: '#717786' }}>{m.duration}</td>
                          <td style={{ padding: '8px 8px' }}>
                            {m.required ? <span style={{ color: '#C0392B', fontWeight: 700, fontSize: 12 }}>{lang === 'en' ? 'Required' : '必修'}</span> : <span style={{ color: '#A0A5B1', fontSize: 12 }}>{lang === 'en' ? 'Elective' : '选修'}</span>}
                          </td>
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        {[
          { key: 'pending', label: lang === 'en' ? 'Pending Supplement Applications' : '待补充申请', value: onboardingApps.filter(a => a.status === 'pending-supplement').length, color: '#C0392B', desc: lang === 'en' ? 'Materials must be supplemented before the deadline' : '需在截止日期前补充材料' },
          { key: 'rejected', label: lang === 'en' ? 'Rejected Applications' : '已驳回申请', value: onboardingApps.filter(a => a.status === 'rejected').length, color: '#717786', desc: lang === 'en' ? 'Rejected for failing admission criteria' : '因不符合准入标准已驳回' },
        ].map(s => (
          <Card key={s.key} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div className="flex items-start justify-between">
                <div style={{ flex: 1 }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge bg={ss.bg} color={ss.color}>{lang === 'en' ? ss.labelEn : ss.label}</Badge>
                    <span style={{ fontSize: 12, color: '#717786' }}>{app.state} · {lang === 'en' ? CHANNEL_TYPE_LABEL_EN[app.applicantType] : CHANNEL_TYPE_LABEL[app.applicantType]}</span>
                  </div>
                  ...
                  {(isPendingSupplement || isRejected) && app.rejectionReason && (
                    <div style={{ padding: '10px 12px', borderRadius: 9, background: isRejected ? 'rgba(180,180,180,0.1)' : 'rgba(255,59,48,0.05)', border: `1px solid ${isRejected ? 'rgba(193,198,215,0.3)' : 'rgba(255,59,48,0.2)'}`, fontSize: 12.5, color: isRejected ? '#555' : '#7A2020', marginBottom: 8 }}>
                      <div className="flex items-center gap-1.5" style={{ fontWeight: 700, marginBottom: 3 }}>
                        ...
                        {isRejected ? (lang === 'en' ? 'Rejection Reason' : '驳回原因') : (lang === 'en' ? 'Supplement Required' : '需要补充')}
                      </div>
                      {lang === 'en' ? (app.rejectionReasonEn ?? app.rejectionReason) : app.rejectionReason}
                      {isPendingSupplement && app.supplementDue && (
                        <div style={{ marginTop: 5, fontWeight: 700, color: '#C0392B', display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Clock size={11} />{lang === 'en' ? 'Supplement due: ' : '补充截止日：'}{app.supplementDue}
                        </div>
                      )}
                    </div>
                  )}
                ...
                <div className="flex flex-col items-end gap-2">
                  {isInReview && (
                    <button onClick={() => setRejectingId(app.id)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 8, fontSize: 12.5, fontWeight: 700, background: 'rgba(255,59,48,0.1)', color: '#C0392B', border: '1px solid rgba(255,59,48,0.25)', cursor: 'pointer' }}>
                      <AlertTriangle size={12} />{lang === 'en' ? 'Reject / Request Supplement' : '驳回 / 要求补充'}
                    </button>
                  )}
                  {isPendingSupplement && (
      ...
      {rejectingId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(24,28,35,0.55)', backdropFilter: 'blur(4px)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="glass-strong" style={{ borderRadius: 20, width: 520, padding: '28px 30px' }}>
            <div className="flex items-center justify-between mb-5">
              <h2 style={{ fontSize: 17, fontWeight: 700, color: '#181C23' }}>{lang === 'en' ? 'Reject / Request Supplemental Materials' : '驳回 / 要求补充材料'}</h2>
              <button className="btn-ghost" style={{ padding: 5 }} onClick={() => setRejectingId(null)}><X size={16} /></button>
            </div>

            <div className="flex flex-col gap-4">
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 8 }}>{lang === 'en' ? 'Action' : '处理方式'}</label>
                {(lang === 'en'
                  ? [['pending-supplement', 'Request supplemental materials (applicant can continue after supplementing by the deadline)'], ['rejected', 'Reject outright (terminates the onboarding process)']]
                  : [['pending-supplement', '要求补充材料（申请人可在截止日期内补充后继续）'], ['rejected', '直接驳回（终止入驻流程）']]
                ).map(([v, l]) => (
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 5 }}>{lang === 'en' ? 'Issue Description / Supplement Requirements *' : '问题说明 / 补充要求 *'}</label>
                <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)} rows={4} placeholder={lang === 'en' ? 'Please describe the rejection reason or required supplemental materials in detail; this notice will be emailed to the applicant…' : '请详细说明驳回原因或需要补充的材料内容，此说明将邮件通知申请人…'} className="input-glass" style={{ width: '100%', fontSize: 13, resize: 'vertical' }} />
              </div>
              {rejectType === 'pending-supplement' && (
                <div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: '#717786', display: 'block', marginBottom: 5 }}>{lang === 'en' ? 'Supplement Deadline' : '补充截止日期'}</label>
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button className="btn-ghost" style={{ padding: '8px 16px' }} onClick={() => setRejectingId(null)}>{lang === 'en' ? 'Cancel' : '取消'}</button>
              <button disabled={!rejectReason} style={{ padding: '8px 20px', borderRadius: 9, fontSize: 13, fontWeight: 700, background: rejectReason ? '#C0392B' : 'rgba(255,59,48,0.3)', color: '#fff', border: 'none', cursor: rejectReason ? 'pointer' : 'not-allowed' }} onClick={() => setRejectingId(null)}>
                {rejectType === 'rejected' ? (lang === 'en' ? 'Confirm Rejection' : '确认驳回') : (lang === 'en' ? 'Send Supplement Request' : '发送补充要求')}
              </button>
            </div>
          </div>
        </div>
...
const TABS = [
  { id: 'applications', icon: <ClipboardList size={15} />, label: '申请总览', labelEn: 'Applications' },
  { id: 'doc-review',   icon: <FileText size={15} />,      label: '资料审核', labelEn: 'Document Review' },
  { id: 'compliance',   icon: <Shield size={15} />,        label: '合规验证', labelEn: 'Compliance Verification' },
  { id: 'contract',     icon: <FileSignature size={15} />, label: '合同签署', labelEn: 'Contract Signing' },
  { id: 'account',      icon: <Key size={15} />,           label: '账号与培训', labelEn: 'Account & Training' },
  { id: 'rejection',    icon: <AlertTriangle size={15} />, label: '驳回与补充', labelEn: 'Rejections & Supplements' },
] as const
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#181C23', letterSpacing: '-0.3px' }}>{lang === 'en' ? 'Channel Onboarding & Admission Management' : '渠道入驻与准入管理'}</h1>
          <p style={{ fontSize: 13, color: '#717786', marginTop: 3 }}>{lang === 'en' ? 'Online Application · Document Review · NIPR / Background / E&O Verification · Contract Signing · Account Setup · Training & Certification' : '在线申请 · 资料审核 · NIPR / 背景 / E&O 验证 · 合同签署 · 账号开通 · 培训认证'}</p>
        </div>
        <div className="flex items-center gap-3">
          {pendingSupplement > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 9, background: 'rgba(255,59,48,0.1)', border: '1px solid rgba(255,59,48,0.25)', fontSize: 12.5, fontWeight: 600, color: '#C0392B' }}>
              <AlertTriangle size={13} />{lang === 'en' ? `${pendingSupplement} pending supplement${pendingSupplement === 1 ? '' : 's'}` : `${pendingSupplement} 份待补充`}
            </div>
          )}
          {bgInProgress > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 9, background: 'rgba(123,63,202,0.08)', border: '1px solid rgba(123,63,202,0.2)', fontSize: 12.5, fontWeight: 600, color: '#7B3FCA' }}>
              <Clock size={13} />{lang === 'en' ? `${bgInProgress} background check${bgInProgress === 1 ? '' : 's'} in progress` : `${bgInProgress} 份背调进行中`}
            </div>
          )}
        </div>
      </div>
